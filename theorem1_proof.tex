...
of the sub-gassian errors) such that for all $\delta>r$ satisfying
\[
...
\[
\sqrt{n_{V}}\delta^{2}\ge a\left(\psi_{\sigma}(4\left\Vert \hat{g}(\cdot|\tilde{\boldsymbol{\lambda}})-g^{*}\right\Vert _{V})\vee4\left\Vert \hat{g}(\cdot|\tilde{\boldsymbol{\lambda}})-g^{*}\right\Vert _{V}\right)
\]
...
We can also bound the summation using Vandegeer Corollary 8.3. First
we check the condition for Corollary 8.3 is satisfied. In particular,
we need to show that for all $s=0,1,2,...$ 
\begin{eqnarray*}
\sqrt{n_{V}}2^{2s+2}\delta^{2} & \ge & a\left(\psi_{\sigma}\left(2^{s+1}\delta\right)\vee\delta\right)
\end{eqnarray*}
...
\begin{eqnarray*}
\sqrt{n_{V}}\delta^{2} & \ge & a\left(\psi_{\sigma}\left(\delta\right)\vee\delta\right)
\end{eqnarray*}
...
$\epsilon$ are independent sub-gaussian random variables
\[
...
Then there is a constant $a>0$ only depends on the characteristics
of the sub-gassian errors.

For any $\delta>0$ such that

\[
\delta^{2}\ge a\max\left\{ \frac{\alpha_{n}^{2}}{n_{V}},\frac{\alpha_{n}}{\sqrt{n_{V}}}\left\Vert \hat{g}(\cdot|\tilde{\boldsymbol{\lambda}})-g^{*}\right\Vert _{V},\frac{1}{n^{2}}\right\} 
\]
...
\[
\alpha_{n}=\sqrt{J}\left(1+\log\left(32Cn\left(\lambda_{max}-\lambda_{min}\right)\right)\right)^{1/2}
\]


where $C_{J}$ is a universal constant.
...
\[
\sqrt{n_{V}}\delta^{2}\ge a\delta\left(\left(J\left(1+\log\left(32C\left(\lambda_{max}-\lambda_{min}\right)n\right)\right)\right)^{1/2}\vee1\right)
\]
...
\[
\delta\ge a\frac{1}{\sqrt{n_{V}}}\left(\left(J\left(1+\log\left(32C\left(\lambda_{max}-\lambda_{min}\right)n\right)\right)\right)^{1/2}\vee1\right)
\]
...
\[
\sqrt{n_{V}}\delta^{2}\ge4a\left\Vert \hat{g}\left(\cdot|\tilde{\boldsymbol{\lambda}}\right)-g^{*}\right\Vert _{V}\left(\left(J\left(1+\log\left(32C\left(\lambda_{max}-\lambda_{min}\right)n\right)\right)\right)^{1/2}\vee1\right)
\]
...
\[
\sqrt{n_{V}}\delta^{2}\ge\alpha\left(\psi_{\sigma}(\delta)\vee\delta\vee\psi_{\sigma}(\tilde{\delta}_{t})\vee\tilde{\delta}_{t}\right)
\]
...
we have 
\[
Pr\left(\left.\sup_{f\in\mathcal{F}(T)}\left|\frac{1}{m}\sum_{i=1}^{m}\epsilon_{i}f(x_{i})\right|\ge\delta\right|T\right)\le C\exp\left(-\frac{m\delta^{2}}{4C^{2}R^{2}}\right)
\]
