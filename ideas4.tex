...
\end{eqnarray*}
The third inequality comes from Lemma 1. We can apply it since for
$s=0,...,S$, 
\[
...
\[
\delta\ge CRJ^{1/2}\left(\frac{1-\log(M/w)+\kappa\log n}{|V|}\right)^{1/2}
\]
...
\[
Pr\left(\sup_{f_{1},f_{2}\in\mathcal{F}}\left|\left(\epsilon,f_{1}-f_{2}\right)\right|_{V}\ge\delta\wedge\|\epsilon\|_{V}\le\sigma\right)\le C\exp\left(-|V|\frac{\delta^{2}}{C^{2}R^{2}}\right)
\]
...
\begin{eqnarray*}
\int_{0}^{R}H^{1/2}\left(u,\bar{\mathcal{F}},\|\cdot\|_{V}\right)du & = & \sqrt{2J}\int_{0}^{R}\left(\log\frac{1}{u}+\log\left(\frac{M}{w}\right)+\kappa\log n\right)^{1/2}du\\
...
\[
\delta\ge CR\sqrt{J}\left(\frac{1-\log(M/w)+\kappa\log n}{|V|}\right)^{1/2}
\]
...
We show that the convergence rate of $\sum_{k=1}^{K}\|g_{\hat{\theta}_{k}(\tilde{\lambda})}-g_{\hat{\theta}_{k}(\hat{\lambda})}\|_{k}^{2}$
is either nearly-parametric or is the same as the optimal convergence
rate:

\[
\sqrt{\sum_{k=1}^{K}\|g_{\hat{\theta}_{k}(\tilde{\lambda})}-g_{\hat{\theta}_{k}(\hat{\lambda})}\|_{k}^{2}}=O_{P}(1)\left(\left(\frac{1-2\log w+\kappa\log n}{\min_{k\in1:K}|D_{k}|}\right)^{1/2}\vee\sqrt{\sum_{k=1}^{K}\|g_{\hat{\theta}_{k}(\tilde{\lambda})}-g_{\theta^{*}}\|_{k}^{2}}\right)
\]
...
\[
\delta\ge O_{p}(1)\left(\frac{1-\log w+\kappa\log n}{\min_{k\in1:K}|D_{k}|}\right)^{1/2}
\]
