...
the training set and $V$ be the validation set. Let the total number
...
\[
\|g_{\hat{\theta}(\hat{\lambda})}-g_{\hat{\theta}(\tilde{\lambda})}\|_{V}\le O_{P}(1)\left(\frac{1-\log w+\kappa\log n}{|V|}\right)^{1/2}
\]
...
\[
\delta\ge\left(\frac{1-\log w+\kappa\log n}{|V|}\right)^{1/2}
\]
...
\|g_{\hat{\theta}(\hat{\lambda})}-g_{\hat{\theta}(\tilde{\lambda})}\|_{V} & \le & 4\left|\frac{(\epsilon,g_{\hat{\theta}(\hat{\lambda})}-g_{\hat{\theta}(\tilde{\lambda})})_{V}}{\|g_{\hat{\theta}(\lambda)}-g_{\hat{\theta}(\tilde{\lambda})}\|_{V}}\right|\\
 & \le & 4\left(\frac{1-\log w+\kappa\log n}{|V|}\right)^{1/2}
\end{eqnarray*}
...
for $k=1,...,K$. Apply Proof Step 1 to each $\mathcal{G}_{\lambda,k}$.
