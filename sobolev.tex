...
\[
\hat{\mathcal{G}}(T)=\left\{ \hat{g}(\cdot|\lambda),\hat{f}(\cdot|\lambda)=\arg\min_{f,g}\frac{1}{2}\|y-(f+g)\|_{T}^{2}+\lambda P(f):\lambda\in\Lambda,g=\sum_{k=1}^{r}\alpha_{k}\psi_{k},f=\int_{0}^{1}\beta_{u}\tilde{\phi}_{u}\right\} 
\]


We will show that
\begin{eqnarray*}
\left\Vert \left(\hat{g}(\cdot|\lambda^{(1)})+\hat{f}(\cdot|\lambda^{(1)})\right)-\left(\hat{g}(\cdot|\lambda^{(2)})+\hat{f}(\cdot|\lambda^{(2)})\right)\right\Vert _{\infty} & \le & |\lambda^{(1)}-\lambda^{(2)}|Gn^{\tau_{min}}\sqrt{\frac{n^{\tau_{min}}}{2}\|\epsilon\|_{T}^{2}+P(f^{*})}
\end{eqnarray*}
...
We will denote it $\hat{g}(\cdot)$ from now on.

Now consider 
\[
h(\cdot)=c\left(\hat{f}(\cdot|\lambda^{(1)})-\hat{f}(\cdot|\lambda^{(2)})\right)
\]
...
The equality follows from the Lemma Sobolev Facts (see below) and
the fact that $P(h)=1$.

We also know that from the Lemma Sobolev Facts that 
\begin{eqnarray*}
\left|\frac{\partial}{\partial m}P\left(\hat{f}(\cdot|\lambda^{(1)})+mh\right)\right| & \le & 2\sqrt{P\left(\hat{f}(\cdot|\lambda^{(1)})+\hat{m}_{h}(\lambda)h\right)P(h)}\\
 & = & 2\sqrt{P\left(\hat{f}(\cdot|\lambda^{(1)})+\hat{m}_{h}(\lambda)h\right)}
\end{eqnarray*}


Combining the two bounds from above, the gradient is bounded by
\begin{eqnarray*}
\left|\frac{\partial}{\partial\lambda}\hat{m}_{h}(\lambda)\right|_{\lambda=\lambda} & \le & n^{\tau_{min}}\sqrt{P\left(\hat{f}(\cdot|\lambda^{(1)})+\hat{m}_{h}(\lambda)h\right)}
\end{eqnarray*}
...
\lambda P\left(\hat{f}(\cdot|\lambda^{(1)})+\hat{m}_{h}(\lambda)h\right) & \le & \frac{1}{2}\|\epsilon\|_{T}^{2}+\lambda^{(1)}P\left(f^{*}\right)+\left(\lambda-\lambda^{(1)}\right)\left(\frac{1}{2\lambda^{(1)}}\|\epsilon\|_{T}^{2}+P\left(f^{*}\right)\right)\\
 & = & \frac{\lambda}{2\lambda^{(1)}}\|\epsilon\|_{T}^{2}+\lambda P\left(f^{*}\right)
\end{eqnarray*}
...
Then by the MVT, there is some $\alpha\in[\lambda^{(1)},\lambda^{(2)}]$
such that 
\begin{eqnarray*}
\|\hat{f}(\cdot|\lambda^{(1)})-\hat{f}(\cdot|\lambda^{(2)})\|_{\infty} & = & \|\left(m_{h}(\lambda^{(2)})-m_{h}(\lambda^{(1)})\right)h\|_{\infty}\\
 & = & \|h\|_{\infty}\left|\lambda^{(2)}-\lambda^{(1)}\right|\left.\frac{\partial}{\partial\lambda}\hat{m}_{h}(\lambda)\right|_{\lambda=\alpha}\\
...
\begin{eqnarray*}
\|\hat{f}_{\ell}(\cdot|\lambda^{(1)})-\hat{f}_{\ell}(\cdot|\lambda^{(2)})\|_{\infty} & \le & G\left\Vert \lambda^{(1)}-\lambda^{(2)}\right\Vert n^{\tau_{min}}\sqrt{\left(n^{\tau_{min}}+Jn^{\tau_{max}+2\tau_{min}}\right)\left(\frac{1}{2}\|\epsilon\|_{T}^{2}+n^{\tau_{max}}\sum_{j=1}^{J}P\left(f_{j}^{*}\right)\right)}
\end{eqnarray*}
...
\begin{eqnarray*}
\|\hat{f}_{\ell}(\cdot|\lambda^{(1)})-\hat{f}_{\ell}(\cdot|\lambda^{(2)})\|_{\infty} & \le & \left\Vert \lambda^{(1)}-\lambda^{(2)}\right\Vert \frac{G^{2}(2G+\|\epsilon\|_{T})n^{2\tau_{min}}}{2}
\end{eqnarray*}
...
is independent of $\lambda$. We will denote it $\hat{g}_{j}$ from
now on.

For every $j=1:J$, define functions 
\[
h_{j}(\cdot)=c\left(\hat{f}_{j}(\cdot|\lambda^{(1)})-\hat{f}_{j}(\cdot|\lambda^{(2)})\right)
\]
...
which is the $\ell$-th column of $M$. By Lemma PSD\_Matrix\_Inverse
...
\max_{j=1:J}P\left(\hat{f}_{j}(\cdot|\lambda^{(1)})\right) & \le & \frac{1}{\lambda_{min}}\left(\frac{1}{2}\|\epsilon\|_{T}^{2}+\sum_{j=1}^{J}\lambda_{j}^{(1)}P\left(f_{j}^{*}\right)\right)\\
 & \le & \frac{1}{\lambda_{min}}\left(\frac{1}{2}\|\epsilon\|_{T}^{2}+\lambda_{max}\sum_{j=1}^{J}P\left(f_{j}^{*}\right)\right)
\end{eqnarray*}


So 
\[
P\left(\hat{f}_{\ell}(\cdot|\lambda^{(1)})+m_{\ell}h_{\ell}\right)\le\left(n^{\tau_{min}}+Jn^{\tau_{max}+2\tau_{min}}\right)\left(\frac{1}{2}\|\epsilon\|_{T}^{2}+n^{\tau_{max}}\sum_{j=1}^{J}P\left(f_{j}^{*}\right)\right)
\]
...
\|\hat{f}_{\ell}(\cdot|\lambda^{(1)})-\hat{f}_{\ell}(\cdot|\lambda^{(2)})\|_{\infty} & = & \|\left(\hat{m}_{\ell}(\lambda^{(2)},h)-\hat{m}_{\ell}(\lambda^{(1)},h)\right)h_{\ell}\|_{\infty}\\
 & = & \|h_{\ell}\|_{\infty}\left|\langle\lambda^{(1)}-\lambda^{(2)},\nabla_{\lambda}\hat{m}_{\ell}(\lambda,h)\rangle_{\lambda=\alpha\lambda^{(1)}+(1-\alpha)\lambda^{(2)}}\right|\\
 & \le & G\left\Vert \lambda^{(1)}-\lambda^{(2)}\right\Vert \left\Vert \nabla_{\lambda}\hat{m}_{\ell}(\lambda,h)\right\Vert _{\lambda=\alpha\lambda^{(1)}+(1-\alpha)\lambda^{(2)}}\\
 & \le & G\left\Vert \lambda^{(1)}-\lambda^{(2)}\right\Vert n^{\tau_{min}}\sqrt{\left(n^{\tau_{min}}+Jn^{\tau_{max}+2\tau_{min}}\right)\left(\frac{1}{2}\|\epsilon\|_{T}^{2}+n^{\tau_{max}}\sum_{j=1}^{J}P\left(f_{j}^{*}\right)\right)}
\end{eqnarray*}
...
\|\hat{f}_{\ell}(\cdot|\lambda^{(1)})-\hat{f}_{\ell}(\cdot|\lambda^{(2)})\|_{\infty} & = & \|\left(\hat{m}_{\ell}(\lambda^{(2)},h)-\hat{m}_{\ell}(\lambda^{(1)},h)\right)h_{\ell}\|_{\infty}\\
 ...
 & \le & G\left\Vert \lambda^{(1)}-\lambda^{(2)}\right\Vert \left\Vert \nabla_{\lambda}\hat{m}_{\ell}(\lambda,h)\right\Vert _{\lambda=\alpha\lambda^{(1)}+(1-\alpha)\lambda^{(2)}}\\
 & \le & \left\Vert \lambda^{(1)}-\lambda^{(2)}\right\Vert G^{2}(2G+\|\epsilon\|_{T})n^{2\tau_{min}}\frac{1}{2}
\end{eqnarray*}
