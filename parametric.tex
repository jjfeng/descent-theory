...
\[
\boldsymbol{\theta}^{*}=\arg\min_{\theta}E_{x,y}\left[\left(y-g(x|\boldsymbol{\theta})\right)^{2}\right]
\]
...
such that for all $j=1,...,J$ and any $\boldsymbol{\theta,\beta},m$,
we have
...
that for all $\boldsymbol{\beta},\boldsymbol{\theta}$, we either
have

\[
\left|\frac{\partial}{\partial m}P_{j}(\boldsymbol{\theta}+m\boldsymbol{\beta})\right|\le K_{j}\|\boldsymbol{\beta}\|_{2}
\]
...
or 
\[
\left\Vert \nabla_{\theta}g_{j}(X_{T,j}|\boldsymbol{\theta})\right\Vert \le K_{j}
\]


(We are bounding the spectral norm of $\nabla_{\theta}g_{j}(X_{T,j}|\boldsymbol{\theta})$.
This case is most relevant to linear models I believe.)
...
\[
d_{k}=\begin{cases}
\left(K+w\sqrt{\left(1+\frac{J\lambda_{max}}{\lambda_{min}}\right)\frac{2C}{\lambda_{min}w}}\right) & \mbox{if }\left|\frac{\partial}{\partial m}P_{k}(\boldsymbol{\theta}+m\boldsymbol{\beta})\right|\le K\|\boldsymbol{\beta}\|\\
\frac{1}{\lambda_{min}}K\sqrt{\left(1+\frac{J\lambda_{max}}{\lambda_{min}}\right)\frac{2C}{\lambda_{min}w}} & \mbox{if }\left\Vert \nabla_{\boldsymbol{\theta}_{k}}g_{k}\left(X_{T,k}|\boldsymbol{\theta}_{k}\right)\right\Vert \le K
\end{cases}
\]
...
\[
\|\boldsymbol{\theta}_{\lambda^{(1)},j}-\boldsymbol{\theta}_{\lambda^{(2)},j}\|\le\left\Vert \boldsymbol{\lambda}^{(2)}-\boldsymbol{\lambda}^{(1)}\right\Vert \lambda_{min}^{-1}w^{-1}d_{max}
\]
...
sum of $g_{j}$ is convex) and the composition of a convex function
with an affine function is convex.
...
and
\begin{equation}
\left\Vert y-\sum_{j=1}^{J}g_{j}(\cdot|\hat{\boldsymbol{\theta}}_{\lambda^{(1)},j}+\hat{m}_{j}(\boldsymbol{\lambda})\boldsymbol{\beta}_{j})\right\Vert _{T}\le\sqrt{\left(1+\frac{J\lambda_{max}}{\lambda_{min}}\right)\frac{2C}{\lambda_{min}w}}
\end{equation}


If combine the assumption 
\[
\left|\frac{\partial}{\partial m}P_{k}(\boldsymbol{\theta}+m\boldsymbol{\beta})\right|\le K\|\boldsymbol{\beta}\|
\]


with (6), we get 
\[
\left|\frac{\partial}{\partial m}_{k}P_{k}(\hat{\boldsymbol{\theta}}_{\lambda^{(1)},k}+m_{k}\boldsymbol{\beta}_{k})+w\langle\boldsymbol{\beta}_{k},\hat{\boldsymbol{\theta}}_{\lambda^{(1)},k}+m_{k}\boldsymbol{\beta}_{k}\rangle\right|_{\boldsymbol{m}=\hat{\boldsymbol{m}}(\boldsymbol{\lambda})}\le\|\boldsymbol{\beta}_{k}\|\left(K+w\sqrt{\left(1+\frac{J\lambda_{max}}{\lambda_{min}}\right)\frac{2C}{\lambda_{min}w}}\right)
\]
...
Then we will need to use the implicit differentiation equation (5).
Rearranging, we get

\begin{eqnarray*}
\left.\frac{\partial}{\partial m_{k}}\left(P_{k}(\hat{\boldsymbol{\theta}}_{\lambda^{(1)},k}+m_{k}\boldsymbol{\beta}_{k})\right)\right|_{\boldsymbol{m}=\boldsymbol{\hat{m}}(\boldsymbol{\lambda})}+w\langle\boldsymbol{\beta}_{k},\hat{\boldsymbol{\theta}}_{\lambda^{(1)},k}+m_{k}\boldsymbol{\beta}_{k}\rangle & = & \frac{1}{\lambda_{k}}\left\langle \left.\nabla_{\boldsymbol{\theta}_{k}}g_{k}\left(\cdot|\boldsymbol{\theta}_{k}\right)\right|_{\boldsymbol{\theta}_{k}=\hat{\boldsymbol{\theta}}_{\lambda^{(1)},k}+m_{k}\boldsymbol{\beta}_{k}}\boldsymbol{\beta}_{k},y-\sum_{j=1}^{J}g_{j}(\cdot|\hat{\boldsymbol{\theta}}_{\lambda^{(1)},j}+m_{j}\boldsymbol{\beta}_{j})\right\rangle _{T}\\
 & \le & \frac{1}{\lambda_{min}}\|\boldsymbol{\beta}_{k}\|\left\Vert \left.\nabla_{\boldsymbol{\theta}_{k}}g_{k}\left(X_{T,k}|\boldsymbol{\theta}_{k}\right)\right|_{\boldsymbol{\theta}_{k}=\hat{\boldsymbol{\theta}}_{\lambda^{(1)},k}+m_{k}\boldsymbol{\beta}_{k}}\right\Vert \left\Vert y-\sum_{j=1}^{J}g_{j}(\cdot|\hat{\boldsymbol{\theta}}_{\lambda^{(1)},j}+\hat{m}_{j}(\boldsymbol{\lambda})\boldsymbol{\beta}_{j})\right\Vert _{T}
\end{eqnarray*}


Plugging in (7), we get 
\[
\left|\frac{\partial}{\partial m}_{k}P_{k}(\hat{\boldsymbol{\theta}}_{\lambda^{(1)},k}+m_{k}\boldsymbol{\beta}_{k})+w\langle\boldsymbol{\beta}_{k},\hat{\boldsymbol{\theta}}_{\lambda^{(1)},k}+m_{k}\boldsymbol{\beta}_{k}\rangle\right|_{\boldsymbol{m}=\hat{\boldsymbol{m}}(\boldsymbol{\lambda})}\le\|\boldsymbol{\beta}_{k}\|\frac{1}{\lambda_{min}}K\sqrt{\left(1+\frac{J\lambda_{max}}{\lambda_{min}}\right)\frac{2C}{\lambda_{min}w}}
\]


Using these upper bounds, we can bound $D_{3}$ by the diagonal matrix
\[
d_{max}diag\left(\left\{ \|\beta\|_{k}\right\} _{k=1}^{J}\right)\succeq D_{3}
\]
...
\|\nabla_{\lambda}\hat{m}_{k}(\lambda)\| & = & \|Me_{k}\|\nonumber \\
 ...
 & \le & \left\Vert d_{max}diag\left(\left\{ \|\beta\|_{k}\right\} _{k=1}^{J}\right)\left(S+D_{1}+D_{2}\right)^{-1}e_{k}\right\Vert \nonumber \\
 & \le & d_{max}\max_{\ell}\|\boldsymbol{\beta}_{\ell}\|\left\Vert \left(S+D_{1}+D_{2}\right)^{-1}e_{k}\right\Vert \nonumber \\
 & \le & d_{max}\max_{\ell}\|\boldsymbol{\beta}_{\ell}\|\left\Vert D_{2}^{-1}e_{k}\right\Vert 
\end{eqnarray}
...
We have 
\begin{eqnarray*}
\|\nabla_{\lambda}\hat{m}_{\ell_{max}}(\lambda)\| & \le & d_{max}\|\boldsymbol{\beta}_{\ell_{max}}\|\left\Vert D_{2}^{-1}e_{\ell_{max}}\right\Vert \\
 & = & d_{max}\|\boldsymbol{\beta}_{\ell_{max}}\|\lambda_{\ell_{max}}^{-1}w^{-1}\|\boldsymbol{\beta}_{\ell_{max}}\|_{2}^{-2}\\
 & \le & d_{max}\|\boldsymbol{\beta}_{\ell_{max}}\|^{-1}\lambda_{min}^{-1}w^{-1}
\end{eqnarray*}
...
\left|\hat{m}_{\ell_{max}}(\boldsymbol{\lambda}^{(2)})-\hat{m}_{\ell_{max}}(\boldsymbol{\lambda}^{(1)})\right| & = & \left|\left\langle \boldsymbol{\lambda}^{(2)}-\boldsymbol{\lambda}^{(1)},\nabla_{\lambda}\hat{m}_{\ell_{max}}(\boldsymbol{\lambda})\right\rangle _{\boldsymbol{\lambda}=\alpha\boldsymbol{\lambda}^{(1)}+(1-\alpha)\boldsymbol{\lambda}^{(2)}}\right|\\
 & \le & \left\Vert \boldsymbol{\lambda}^{(2)}-\boldsymbol{\lambda}^{(1)}\right\Vert d_{max}\lambda_{min}^{-1}w^{-1}\|\boldsymbol{\beta}_{\ell_{max}}\|^{-1}
\end{eqnarray*}
...
\[
\max_{k}\|\boldsymbol{\theta}_{\lambda^{(1)},k}-\boldsymbol{\theta}_{\lambda^{(2)},k}\|=\|\boldsymbol{\beta}_{\ell_{max}}\|\le\left\Vert \boldsymbol{\lambda}^{(2)}-\boldsymbol{\lambda}^{(1)}\right\Vert d_{max}\lambda_{min}^{-1}w^{-1}
\]
...
Suppose $\Lambda\subseteq\mathbb{R}^{p}$. We suppose that for almost
every dataset $\left(X,y\right)$, the following hold:

\textbf{Assumption (1):} Let the penalized training criterion be denoted
$L_{T}(\cdot,\boldsymbol{\lambda})$. Denote the differentiable space
of $L_{T}(\cdot,\boldsymbol{\lambda})$ at any point $\boldsymbol{\theta}$
as $\Omega^{L_{T}(\cdot,\boldsymbol{\lambda})}\left(\boldsymbol{\theta}\right)$.
Suppose there is a set $\Lambda_{smooth}\subseteq\Lambda$ such that
for every $\boldsymbol{\lambda}\in\Lambda_{smooth}$, the following
conditions hold

\textbf{Cond 1:} The differentiable space of the training criterion
at $\hat{\boldsymbol{\theta}}(\boldsymbol{\lambda})$, denoted $\Omega^{L_{T}(\cdot,\boldsymbol{\lambda})}\left(\hat{\boldsymbol{\theta}}_{\lambda}\right)$,
is a local optimality space.

\textbf{Cond 2:} The training criterion $L_{T}(\cdot,\cdot)$ restricted
to $\Omega^{L_{T}(\cdot,\cdot)}\left(\hat{\boldsymbol{\theta}}_{\lambda},\boldsymbol{\lambda}\right)$
...
\textbf{Assumption} \textbf{(3):} All the conditions that bound the
spectrum of $P_{j}$ or $g_{j}$ only need to apply when the directional
derivatives exist. That is, the condition on the spectrum of the penalty
derivative is now
...
derivative to
\[
\left\Vert \nabla_{\boldsymbol{\theta}}g_{j}\left(\boldsymbol{\theta}\right)\right\Vert \le K\mbox{ if }\nabla_{\boldsymbol{\theta}}g_{j}\left(\boldsymbol{\theta}\right)\mbox{exists}
\]
...
We define the length of $\mathcal{L}(\boldsymbol{\lambda^{(1)}},\boldsymbol{\lambda^{(2)}})$
covered by set $A$ as 
\[
d_{\boldsymbol{\lambda^{(1)}},\boldsymbol{\lambda^{(2)}}}\left(A\right)=\mu\left(A\cap\mathcal{L}(\boldsymbol{\lambda^{(1)}},\boldsymbol{\lambda^{(2)}})\right)
\]
...
\[
\max_{\{\boldsymbol{\ell}^{(i)}\}_{i=1}^{\infty}}d_{\boldsymbol{\lambda^{(1)}},\boldsymbol{\lambda^{(2)}}}\left(\cup_{i=1}^{\infty}B(\boldsymbol{\ell}^{(i)})\right)=\mu\left(\mathcal{L}\left(\boldsymbol{\lambda^{(1)}},\boldsymbol{\lambda^{(2)}}\right)\right)
\]


Let 
\[
\left\{ \boldsymbol{\ell}_{max}^{(i)}\right\} _{i=1}^{\infty}=\arg\max_{\left\{ \boldsymbol{\ell}^{(i)}\right\} }d_{\boldsymbol{\lambda^{(1)}},\boldsymbol{\lambda^{(2)}}}\left(\cup_{i=1}^{\infty}B(\boldsymbol{\ell}^{(i)})\right)
\]


Let $P$ be the intersections of the boundary of $B(\boldsymbol{\ell}^{(i)})$
with the line segment $\mathcal{L}(\boldsymbol{\lambda^{(1)}},\boldsymbol{\lambda^{(2)}})$:
\[
P=\left\{ \cup_{i=1}^{\infty}\mbox{bd}B(\boldsymbol{\ell}^{(i)})\cap\mathcal{L}(\boldsymbol{\lambda^{(1)}},\boldsymbol{\lambda^{(2)}})\right\} \cup\{\boldsymbol{\lambda}^{(1)},\boldsymbol{\lambda}^{(2)}\}
\]


The point $p\in P$ can be expressed as $\alpha_{p}\boldsymbol{\lambda^{(1)}}+(1-\alpha_{p})\boldsymbol{\lambda^{(2)}}$
...
over line segment $\mathcal{L}\left(\boldsymbol{p^{(i)},p^{(i+1)}}\right)$
must be constant. 

Now we apply the smoothness result in Section 1 or 2 over every line
segment $\mathcal{L}\left(\boldsymbol{p^{(i)},p^{(i+1)}}\right)$.
We can come up with the following equivalent definition for $\hat{\boldsymbol{\theta}}(\boldsymbol{\lambda})$:
There is an orthonormal matrix $U^{(i)}$ such that for all $\boldsymbol{\lambda}\in\mathcal{L}\left(\boldsymbol{p^{(i)},p^{(i+1)}}\right)$
\[
...
where the training criterion is smooth over $\mathcal{L}\left(\boldsymbol{p^{(i)},p^{(i+1)}}\right)$
...
directional derivatives along the columns of $U$. We can establish
that there is a constant $c>0$ such that for every tuple of points
$\left(\boldsymbol{p^{(i)},p^{(i+1)}}\right)$ from $i=1,2...$, we
have 
\[
...
\begin{eqnarray*}
\|\boldsymbol{\hat{\theta}_{\lambda^{(1)}}}-\boldsymbol{\hat{\theta}_{\lambda^{(2)}}}\|_{2} & \le & \sum_{i=1}^{\infty}\|\boldsymbol{\hat{\theta}}_{p^{(i)}}-\boldsymbol{\hat{\theta}}_{p^{(i+1)}}\|_{2}\\
...
\[
\left\{ \hat{\boldsymbol{\theta}}_{\lambda}=\arg\min_{\theta}\frac{1}{2}\|y-\sum_{j=1}^{J}N_{T,j}\boldsymbol{\theta}_{j}\|_{T}^{2}+\sum_{j=1}^{J}\lambda_{j}P_{j}(\boldsymbol{\theta}_{j}):\lambda\in\Lambda\right\} 
\]
...
parameters
\[
\left\{ \hat{\boldsymbol{\theta}}_{\lambda}=\arg\min_{\theta}\frac{1}{2}\|y-\sum_{j=1}^{J}N_{T,j}\boldsymbol{\theta}_{j}\|_{T}^{2}+\sum_{j=1}^{J}\lambda_{j}\left(P_{j}(\boldsymbol{\theta}_{j})+\frac{w}{2}\|\boldsymbol{\theta}_{j}\|_{2}^{2}\right):\lambda\in\Lambda\right\} 
\]
...
\[
\left\Vert \sum_{j=1}^{J}\hat{g}_{j}(x_{j}|\lambda^{(1)})-\hat{g}_{j}(x_{j}|\lambda^{(2)})\right\Vert _{\infty}\le\left\Vert \boldsymbol{\lambda}^{(2)}-\boldsymbol{\lambda}^{(1)}\right\Vert _{2}J\sqrt{B}\lambda_{min}^{-1}w^{-1}\left(\frac{1}{\lambda_{min}}B\sqrt{\left(1+\frac{J\lambda_{max}}{\lambda_{min}}\right)\frac{2C}{\lambda_{min}w}}\right)
\]
