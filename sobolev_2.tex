...
up to one at all points within the boundary of the knots. (I might
be wrong regarding the relationship between the number of knots and
the number of basis functions, but the number of basis functions should
be linear in the number of knots)
...
The maximum eigenvalue of $N_{T}^{T}$ is bounded by the maximum row
sum. Since each row of $N_{T}^{T}$ is the value of each of the normalized
B-spline basis functions at a given point, each row of $N_{T}^{T}$
sums to one. Therefore by the Gershgorin circle theorem ({*}), the
maximum eigenvalue of $N_{T}^{T}$ is bounded by 1. Hence
