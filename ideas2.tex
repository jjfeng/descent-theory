...
(\textbf{NOTE: A circular definition for optimal lambda!} You choose
...
then
\[
\|\sum_{j=1}^{J}g_{j}-g_{j}^{*}\|_{T}=O_{p}\left(\tilde{\lambda}_{T}\right)\left(J^{\frac{\alpha_{max}}{2+\alpha_{max}}}\vee\max_{j\in1:J}\left\{ J^{\frac{\alpha_{max}-v_{j}+1}{2v_{j}+v_{j}\alpha_{max}-2\alpha_{max}}}\right\} \right)\max_{j\in1:J}\left(I_{j}^{v_{j}}(g_{j}^{*})\right)^{1/2}
\]
...
\|\sum_{j=1}^{J}\hat{g}_{j}-g_{j}^{*}\|_{T} & \le & O_{p}\left(n^{-1/(2+\alpha_{max})}J^{\alpha_{max}/(2+\alpha_{max})}\right)I_{(j)}^{\alpha_{max}/(2+\alpha_{max})}(g_{(j)}^{*})\\
 & \le & O_{p}(\lambda)J^{\alpha_{max}/(2+\alpha_{max})}\sup_{j\in1:J}\left(I_{j}^{v_{j}}(g_{j}^{*})\right)^{1/2}
\end{eqnarray*}
