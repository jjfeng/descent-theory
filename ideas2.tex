...
\subsection*{Result 3 again: Single $\lambda$, Multiple Penalties, Optimal $\tilde{\lambda}_{T}$
over $X_{T}$}
...
Suppose $v_{j}>\frac{2\alpha}{2+\alpha}$ for all $j$. 
...
Since either $I_{j}(\hat{g}_{j})\le I_{j}^{v_{j}}(\hat{g}_{j})$ or
$1\ge I_{j}(\hat{g}_{j})$, then
