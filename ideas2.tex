...
\[
I_{(k)}^{2}(g_{\lambda})=M\|Proj_{\mathcal{H}}(g_{\lambda})\|^{2}+M_{0}
\]


where $\mathcal{H}$ is the Reproducing Kernel Hilbert space from
Noah's document and $Proj_{\mathcal{H}}(g_{\lambda})$ means we are
projecting $g_{\lambda}$ onto $\mathcal{H}$.
...
\[
Proj_{\mathcal{H}}(g_{\lambda})=\Omega\alpha,\mbox{ }I_{(k)}^{2}(g_{\lambda})=\alpha^{T}\Omega\alpha
\]
...
I_{(k)}^{2}(g_{\lambda}) & = & \alpha^{T}\Omega\alpha\\
 ...
 & = & O_{p}(n^{-\tau})\|\Omega\alpha\|_{2}^{2}\\
 & \le & \|\Omega\alpha\|_{2}^{2}+M_{0}\\
 & = & \|Proj_{\mathcal{H}}(g_{\lambda})\|_{2}^{2}+M_{0}
\end{eqnarray*}


(That is, for sufficiently large $n$, we bound the penalty with the
constant $M_{0}$ if $\tau<0$ and we bound the penalty with $\|Proj_{\mathcal{H}}(g_{\lambda})\|_{2}^{2}$
if $\tau>0$)
\end{document}
