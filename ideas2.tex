...
\textbf{Assumption 2: }Show that there exist constants $M,M_{0}$
s.t. the penalty is bounded by the squared L2 norm: 
\[
I_{(k)}^{2}(g_{\lambda})\le M\|g_{\lambda}\|^{2}+M_{0}
\]
...
the Reproducing Kernel Hilbert space $\mathcal{H}$, we have 
\[
...
some constant $\tau$. Then for large enough $n$ 
\begin{eqnarray*}
I_{(k)}^{2}(g_{\lambda}) & = & \alpha^{T}\Omega\alpha\\
 ...
 & \le & \|Proj_{\mathcal{H}}(g_{\lambda})\|_{2}^{2}+\|Proj_{\mathcal{H}^{\perp}}(g_{\lambda})\|_{2}^{2}+M_{0}\\
