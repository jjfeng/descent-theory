...
of the semi-norm.
\item We assume function class $\mathcal{G}$ satisfies for some constant
$\tilde{M}$, $\sum_{j=1}^{J}\|g_{j}\|^{2}\le\tilde{M}\|\sum_{j=1}^{J}g_{j}\|^{2}$
for all $j$. That is, no single function can dominate the behavior
of the sum. Actually, ``HIGH-DIMENSIONAL ADDITIVE MODELING'' (Vandegeer
2009) has a similar compatibility condition.
\end{itemize}
...
\textbf{Special Assumption: }Assume the function class is 
\[
\mathcal{G}=\left\{ \sum_{j=1}^{J}g_{j}:\mbox{ }g_{j}\in\mathcal{G}_{j},\sum_{j=1}^{J}\|g_{j}\|^{2}\le\tilde{M}\|\sum_{j=1}^{J}g_{j}\|^{2}\right\} 
\]


for some constant $\tilde{M}$. (Assume $\sum_{j=1}^{J}g_{j}^{*}\in\mathcal{G}$)
...
\[
\|\sum_{j=1}^{J}\hat{g}_{\hat{\lambda},j}-\hat{g}_{\tilde{\lambda},j}\|_{V}=O_{p}\left(n^{-1/(2+\alpha)}\right)\left(\left(\sum_{j=1}^{J}I_{j}(g_{j}^{*})\right)^{\alpha/(2+\alpha)}+\sum_{j=1}^{J}I_{j}(g_{j}^{*})+\left\Vert \sum_{j=1}^{J}g_{j}^{*}\right\Vert _{V}^{\alpha/2(2+\alpha)}\right)
\]
...
By the assumptions,
\begin{eqnarray*}
\sum_{j=1}^{J}I_{j}(\hat{g}_{\hat{\lambda},j}) & \le & M\sum_{j=1}^{J}\|\hat{g}_{\hat{\lambda},j}\|_{V}^{2}+M_{0}\\
 & \le & M\tilde{M}\|\sum_{j=1}^{J}\hat{g}_{\hat{\lambda},j}\|_{V}^{2}+M_{0}
\end{eqnarray*}


Then
\begin{eqnarray*}
\left\Vert \sum_{j=1}^{J}\hat{g}_{\tilde{\lambda},j}-\hat{g}_{\hat{\lambda},j}\right\Vert _{V}^{1+\alpha/2} & \le & O_{p}(n^{-1/2})\left(M\tilde{M}\|\sum_{j=1}^{J}\hat{g}_{\hat{\lambda},j}\|_{V}^{2}+M_{0}\right)^{\alpha/2}
\end{eqnarray*}


If $M_{0}$ is the biggest, we're done. Otherwise,

\begin{eqnarray*}
\left\Vert \sum_{j=1}^{J}\hat{g}_{\tilde{\lambda},j}-\hat{g}_{\hat{\lambda},j}\right\Vert _{V}^{1+\alpha/2} & \le & O_{p}(n^{-1/2})\|\sum_{j=1}^{J}\hat{g}_{\hat{\lambda},j}\|_{V}^{\alpha}\\
 & \le & O_{p}(n^{-1/2})\left(\left\Vert \sum_{j=1}^{J}\hat{g}_{\tilde{\lambda},j}-\hat{g}_{\hat{\lambda},j}\right\Vert _{V}+\left\Vert \sum_{j=1}^{J}\hat{g}_{\tilde{\lambda},j}-g_{j}^{*}\right\Vert _{V}+\left\Vert \sum_{j=1}^{J}g_{j}^{*}\right\Vert _{V}\right)^{\alpha}
\end{eqnarray*}
