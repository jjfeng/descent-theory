...
\usepackage{xr}
\externaldocument{multi-penalties-theory}
...
\title{
Oracle inequalities for tuning hyper-parameters via validation sets with applications to penalized regression: Supplement
}
...
\begin{proof}
	...
	\end{equation}
	for $$K_{n,1} = \left[J\left(1+\log\left(128\sqrt{n_V} GC\lambda_{max}  \right)\right)\right]^{1/2}$$ and $$K_{n,2} = \log n_V 2GJ\left(1+\log\left(128GC\lambda_{max}\right)\right)$$
