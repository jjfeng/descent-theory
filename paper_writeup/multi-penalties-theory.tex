...
\begin{equation}
\label{cv_lambda}
\hat{\lambda} = \arg\min_{\lambda\in\Lambda} \frac{1}{2}  \| y-\hat{g}(\cdot | \boldsymbol \lambda) \|_{V}^{2}
\end{equation} 
...
The bound is based on the basic inequality (cite?). Let $\tilde{\boldsymbol \lambda}$ be the oracle penalty parameters. From the definition of $\hat{\boldsymbol \lambda}$, we have
\begin{equation}
\label{basic_ineq}
...
\hat{g}(\cdot | \lambda, D_{-k})&=&\arg\min_{g\in\mathcal{G}} \frac{1}{2} \| y-g \|_{-k}^{2} + \sum_{j=1}^J \lambda_j P_j^{v_j}(g) + \frac{w}{2} \|g\|^2
\end{eqnarray}

In traditional cross-validation, the final model is retrained on all the data with $\hat{\lambda}$. However, bounding its generalization error requires additional regularity assumptions (CITE mitchell). Instead, consider an ``averaged version of cross-validation":
\begin{equation}
\hat{g}_{ACV}(\cdot) = \frac{1}{K} \sum_{k=1}^K \hat{g}(\cdot | \hat{\boldsymbol \lambda}, D_{-k})
\end{equation}
The following theorem bounds the generalization error of $\hat{g}_{ACV}$. For any function $h$, define $\| h \|^2 = \int h^2(x) d\mu(x)$.
...
Equipped with the conditions above, we can characterize the smoothness of the fitted functions when the penalties are nonsmooth. In fact the result is exactly the same as Lemma \ref{lemma:smooth_penalty}. The proof is given in Section \ref{sec:proofs}. 
...
\begin{condition}
\label{condn:param2}
The functions are $Lp^r$-Lipschitz in the model parameters: 
\begin{equation}
...
Condition \ref{condn:param2} is also quite reasonable (I think). For example, it is satisfied in linear regression when the covariates are bounded.
...
+\frac{\lambda_{\max}}{\lambda_{\min}}\sum_{j=1}^{J}P\left(g_j^{*}\right)}G
\end{equation}
\end{lemma}
