...
Suppose there exist constants $C$ such that for any dataset $D^{n_T}$, $\hat{g}_{D^{n_T}}(\cdot | \boldsymbol \lambda)$ is $C$-smoothly-parameterized by $\boldsymbol \lambda$ over $\Lambda$ with respect to $\| \cdot \|_\infty$.
...
Suppose there are constants $C,\kappa$ such that for any dataset $D^{(n_T)}$, $\hat{g}_{D^{(n_T)}}(\cdot | \boldsymbol \lambda)$ is $Cn^\kappa$-smoothly-parameterized by $\boldsymbol \lambda$ over $\Lambda$ with respect to $\| \cdot \|_\infty$.
...
Theorem \ref{train_val_thrm} requires that $\hat{g}(\cdot | \boldsymbol{\lambda})$ is $C$-smoothly-parameterized by $\boldsymbol{\lambda}$ over $\Lambda$ with respect to $\| \cdot \|_V$. We show that additive models satisfy this smoothness condition.
