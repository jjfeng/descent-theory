...
The ridge penalty is used in our proofs to ensure that the fitted functions are ``well-conditioned.'' In practice, $w$ can be chosen small enough such that the fitted models for the original problem and the perturbed ridge problem are indistinguishable. In Lemma \ref{lemma:ridge_perturb_smooth}, we quantify how small $w$ needs to be. We find that $w$ often just needs to be polynomial in $n$, which means that the order of the convergence rate in Theorem \ref{thrm:train_val} and \ref{thrm:kfold} is unchanged.
...
\end{equation}
We will show that $\hat{\boldsymbol{\theta}}(\boldsymbol{\lambda})$ is Lipschitz with respect to $\|\cdot\|_2$. So if $g_j(\cdot| \boldsymbol{\theta}^{(j)})$ is Lipschitz in $\boldsymbol{\theta}^{(j)}$ with respect to $\| \cdot \|_\infty$ for all $j=1,..,J$, then both Theorems \ref{thrm:train_val} and \ref{thrm:kfold} are satisfied.\footnote{Many popular estimators are Lipschitz since we usually construct the model by taking a linear combination of some basis functions.}

\subsubsection{Parametric regression with smooth penalties}
We begin with the simple case where all the penalty functions are smooth. We later generalize these result to include non-smooth penalty functions like the lasso and group lasso.

The following lemma states that the fitted models are Lipschitz in the penalty parameter vector.
\begin{lemma}
	\label{lemma:param_add}
	Define $\hat{\boldsymbol{\theta}}^{(j)}\left (\boldsymbol{\lambda}\right )$ as in \eqref{eq:param_add_models_ridge}.
	Suppose that for all $j=1,..,J$, $g_j(\cdot| \boldsymbol{\theta}^{(j)})$ are $L$-Lipschitz in $\boldsymbol{\theta}^{(j)}$ with respect to $\| \cdot \|_\infty$.
	
	Let $\lambda_{\max} > \lambda_{\min} > 0 $.
	For any $\boldsymbol{\lambda}^{(1)}, \boldsymbol{\lambda}^{(2)} \in \Lambda \coloneqq \left [ \lambda_{\min}, \lambda_{\max} \right ]^J$, we have for all $j = 1,...,J$
	\[
	\left \|
	g_j\left (\cdot \middle | \hat{\boldsymbol{\theta}}^{(j)} \left (\boldsymbol{\lambda}^{(1)} \right ) \right ) - 
	g_j \left ( \cdot \middle | \hat{\boldsymbol{\theta}}^{(j)}\left (\boldsymbol{\lambda}^{(2)} \right ) \right )
	\right \|_{\infty}
	\le
	\left \|
