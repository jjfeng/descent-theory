...
The supremum of empirical processes can be bounded using the complexity of the model class $\mathcal{G}(T)$. Complexity can be measured in a number of ways; we will use metric entropy in this paper. A more thorough review of empirical process theory is presented in Section \ref{sec:proofs}. In this section, we only concern ourselves with smoothly-parameterized functions. More formally, a functions is $C$-smoothly-parameterized by $\boldsymbol \lambda$ over $\Lambda$ if
\begin{equation}
...
\begin{theorem}
\label{train_val_thrm} (give the theorem with $\lambda_{min}$ and $\lambda_{max}$ )
Suppose $\Lambda=[n_{V}^{-t_{min}},n_{V}^{t_{max}}]^{J}$.

Suppose that if $\|\epsilon\|_{T}\le2\sigma$, there are constants
$C,\kappa$ such that for any $u>0$, we have for all $\lambda\in\Lambda$ (give the theorem with $C$ rather than $Cn^\kappa$)
\[
...
$$
$$
	\le c_{3}\exp\left(-\frac{n_{V}\delta^{4}}{c_{3}^{2}\left\Vert \hat{g}(\cdot|\tilde{\boldsymbol{\lambda}})-g^{*}\right\Vert _{V}^{2}}\right)+c_{3}\exp\left(-\frac{n_{V}\delta^{2}}{c_{3}^{2}}\right)+Pr\left(\|\epsilon\|_{T}\le2\sigma\right)
$$
...
Technically we would like to tune the penalty parameters over the entire range $\Lambda = \mathbb{R}^J_+$, but $\hat g(\cdot | \boldsymbol{\lambda})$ can be very ill-behaved under such general conditions. A close approximation is if we ensure that $\Lambda$ includes the penalty parameter
\begin{equation}
...
\end{equation}
As shown in Lemma ? (Or cite van de geer), $\tilde{\boldsymbol{\lambda}}_{\mathbb{R}_+}$ shrinks at some polynomial rate $O_p(n^{-\omega})$, so the lower limit of $\Lambda$ just needs to shrink at a faster polynomial rate $O_p(n^{-t_{\min}})$ where $t_{\min} > \omega$. Suppose $\Lambda = [n^{-t_{min}},n^{t_{max}}]^{J}$. Under this condition, we find that the fitted models $\hat g({\cdot | \boldsymbol{\lambda}})$ for all our examples in Section \ref{sec:entropy} are $Cn^{-\kappa}$-smoothly parameterized by $\boldsymbol{\lambda}$. We can now apply Theorem \ref{train_val_thrm} to this special case. For ease of interpretation, we present the results in asymptotic notation this time:
...
Therefore the model error is upper bounded by the sum of three terms: the oracle error, the near-parametric term in \ref{eq:asym_train_val_theorem2}, and the geometric mean of the two. We call \ref{eq:asym_train_val_theorem2} near-parametric since the convergence rate for a $J$-dimensional parametric model is usually $(J/n)^{1/2}$ and \ref{eq:asym_train_val_theorem2} has a $\log n$ term in the numerator. The $\log n$ term appears because the range of the penalty parameters grows at a polynomial rate in the number of samples. Since $\log n$ grows very slowly, its contribution is nearly negligible.
...
2. There is an interesting log n term. it appears because the range of Lambda grows with $n_V$ The $\log n_V$ terms are the result of increasing the range of $\Lambda$ at a polynomial rate. (Not true). Increasing the range of $\Lambda$ in important to ensure fast convergence rates. (True... but is this important to put here)

3. An interesting geometric mean has appeared. We analyze if this is truly necessary in the simuations... though i still have no idea if it is.

4. Mention no one else has a sharp oracle inequality. Possibly caus they are dealing with generalization error rather than the validation error.
