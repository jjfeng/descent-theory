...
It can be shown that the estimate is a spline. According to Vandegeer (CITE), the oracle convergence rate is
\begin{equation}
\| \hat{f}_{\tilde \lambda} - f^* \| = O_p(n^{-\frac{s}{2s+1}}), \| \hat{g}_{\tilde \lambda} - g^* \| = O_p(n^{-\frac{t}{2t+1}})
\end{equation}
However, one would need to know the values of the Sobolev penalties of $f^*$ and $g^*$ in order
