...
Theorems \ref{thrm:train_val} and \ref{thrm:kfold} require the fitted functions $\hat{g}(\cdot | \boldsymbol{\lambda})$ to be Lipschitz when the norm of the error terms is bounded. We now show that additive models, in both the parametric and nonparametric setting, are $C$-Lipschitz:
\begin{equation}
\label{eq:lipschitz}
	\left \|
	\hat{g}\left (\cdot \middle | \boldsymbol{\lambda}^{(1)} \right ) - 
	\hat{g}\left (\cdot \middle | \boldsymbol{\lambda}^{(2)} \right ) 
	\right \|
	\le
	C
	\left \|
...
\end{equation}
Recall  that in Theorems \ref{thrm:train_val} and \ref{thrm:kfold}, the Lipschitz constant $C$ contributes a $\log C$ term in the convergence rate. Therefore the convergence rate is unchanged as long as $C$ is polynomial in $n$.
...
Finally, we note that additive models are not the only problems where the estimators are Lipschitz. In the Appendix, we show that regression problems where we fit a single model $g(\cdot | \boldsymbol{\theta})$ with multiple, individually-scaled penalties $P_j(\boldsymbol{\theta})$ also satisfies \eqref{eq:lipschitz}.

\subsection{Parametric additive models}
\label{sec:param_add_models}

Our first example is penalized regression for parametric additive models, which can be written as
\begin{equation}
...
%\left \{ \hat{\boldsymbol{\theta}}^{(j)}(\boldsymbol{\lambda}) \right \}_{j=1}^J = \argmin_{\boldsymbol{\theta }^{(j)}\in \mathbb{R}^{p_j}} 
%\frac{1}{2} \left  \| y -  \sum_{j=1}^J g_j(X| \boldsymbol{\theta}^{(j)}) \right \|^2_T 
%+ \sum_{j=1}^J \lambda_j \left ( P_j(\boldsymbol{\theta}^{(j)}) + \frac{w}{2} \| \boldsymbol{\theta}^{(j)} \|^2_2 \right )
%\end{equation}
...
We begin with the simple case where all the penalty functions are smooth. We later generalize the result to include non-smooth penalty functions like the lasso and group lasso.

The following lemma states that the fitted models are Lipschitz in the penalty parameter vector.
\begin{lemma}
...
\begin{definition}
	$S$ is a local optimality space for a convex function $f(\cdot, \boldsymbol \lambda)$ over the (\textcolor{red}{open? closed?}) $W \subseteq \Lambda$ if for every $\boldsymbol \lambda \in W$,
	\begin{equation}
...
Equipped with the conditions above, we can characterize the smoothness of the fitted functions when the penalties are non-smooth. In fact the result is exactly the same as Lemma \ref{lemma:param_add}.
...
We now generalize the previous section to nonparametric additive models. We consider estimators of the form
\begin{equation}
...
Suppose the penalty functions $P_{j}$ is twice Gateaux differentiable and convex over $\mathcal{G}$.
...
Interestingly, Theorem \ref{thrm:train_val} states that the convergence rate should be a geometric mean of the near-parametric rate and the oracle rate. Since the oracle rate here is constant, we should be observing a convergence rate of $n_V^{-1/2}$. We in fact observe a convergence rate of $n_V^{-1}$. Our hypothesis that our convergence rate bounds are not tight. It is possible that problems like the Sobolev are well-behaved enough that the geometric term can be completely dropped.
