...
The goal of this paper is to characterize the performance of models when the hyper-parameters must be tuned by some split-sample validation procedure. We are particularly interested in an open question raised in \citet{bengio2000gradient}: what is the ``amount of overfitting... when too many hyper-parameters are optimized''? and in addition, when do we reach ``too many hyper-parameters?'' In this paper we show that actually a large number of hyper-parameters can be tuned without overfitting. In fact, if an oracle estimator converges at rate $1/R(n)$, then the number of hyper parameters can grow at roughly a rate of $n/R(n)$ up to log terms without affecting the convergence rate. In practice, for penalized regression, this means that one can propose and tune over much more complex models than are currently often used. In addition, this explains, in part, the strong performance of machine-learning-based models that tune many more hyper-parameters than standard statistical models \textcolor{red}{Citations?}.
