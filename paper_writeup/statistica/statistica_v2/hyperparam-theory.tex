...
our goal is to find some hyper-parameter $\hat{\boldsymbol{\lambda}}$ that attains the oracle risk
\[
\min_{\lambda \in \Lambda} \left\|g^{*} - \hat{g}\left(\boldsymbol{\lambda|}T\right)\right\|^2.
\]
...
\begin{definition}
	...
	The operator $g: \Lambda \mapsto \mathcal{G}$ is $C_\Lambda$-Lipschitz in $\boldsymbol{\lambda}$ over $\Lambda$ if there is some function $C_{\Lambda}:\mathcal{X} \mapsto \mathbb{R}^+$ such that for all $\boldsymbol{x} \in \mathcal{X}$,
	\begin{equation}
...
For instance, Section \ref{sec:examples} shows that examples of penalized regression models that satisfy this assumption.
...
\begin{assump}
	...
	For any $n_T \in \mathbb{N}$, dataset $T$ of size $n_T$, and $\boldsymbol{\lambda}^{(1)}, \boldsymbol{\lambda}^{(2)} \in \Lambda$, we have
	\begin{align}
	\left |
...
%JF english...
%JF - also maybe just cut this paragraph for space?
