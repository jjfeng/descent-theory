...
% JF and alpha = 1...? thats also used everywhere
...
\begin{lemma}
	...
	\sqrt{\frac{1}{2}\|\epsilon\|_{T}^{2}+C_{\Lambda}^{*}}
	\right )
	\left(
	\max_{j \in 1:J} \|\ell_j(\cdot)\|_T
	\right)
	\sum_{j=1}^J \ell_j(x).
	\end{equation}
\end{lemma}
...
\begin{example}[Multiple ridge penalties]
	...
	&\le \left\Vert g^* - \hat{g}^{(n_T)}( \tilde{\boldsymbol{\lambda}} | T) \right \Vert^2_{V}\\
	& + O_p\left (
	\frac{J \kappa \log(C_{\Lambda, D^{(n_T)}} \|\ell(\cdot)\|_V n + 1)}{n_{V}}
	\right ) \\
	& + O_p\left(
	\sqrt{\frac{J \kappa \log(C_{\Lambda, D^{(n_T)}} \|\ell(\cdot)\|_V n + 1)}{n_{V}}\left\Vert g^* - \hat{g}^{(n_T)}( \tilde{\boldsymbol{\lambda}} | T)\right\Vert_{V}^2}
	\right ).
	\end{split}
	\end{align}
\end{example}

\begin{example}[Sobolev penalties]
	...
	For this example, we suppose that the $x_{ij}$ in the training set are uniformly spaced in the interval from 0 to 1.
	The covariates in the validation set can be arbitrarily spaced.
	...
	Thus we can re-express \eqref{eq:smoothing_spline} as
	\begin{align}
	\hat{g}(\boldsymbol{\lambda})|_{D^{(n_T)}}
	& = \argmin_{g \equiv \sum_{j=1}^J g_j \in \mathbb{R}^{n_T}}
	\frac{1}{2} \sum_{i\in D^{(n_T)}}
	\left(
	y_i - \sum_{j=1}^J g_{ji}
	\right)^2
	+ \sum_{j=1}^{J} \frac{\lambda_j}{n_T-1} g_j^\top K g_j
	\label{eq:sobolev_rewrite}
	\end{align}
	where the matrix $K$ is a positive semi-definite matrix defined in \citet{green1993nonparametric}.
	We can also rewrite this problem using matrix notation
	\begin{align}
	\hat{g}(\boldsymbol{\lambda})|_{D^{(n_T)}}
	& = \argmin_{g \equiv \sum_{j=1}^J g_j \in \mathbb{R}^n}
	\frac{1}{2}
	\left \|
	y - I^{(J)} g
	\right \|^2
	...
	\diag \left (
	\left \{
	\frac{\lambda_j}{n_T-1} K
	\right \} \right ) g
	\end{align}
	where $I^{(J)}$ is a concatenation of $J$ identity matrices.
	One can show (using properties of generalized inverse matrices/projection matrices) that the unique solution to \eqref{eq:smoothing_spline} is
	\begin{align}
	\hat{g}(\boldsymbol{\lambda})|_{D^{(n_T)}}
	= I^{(J)}
	\left (
	I^{(J)\top} I^{(J)} + \diag \left (
	\left \{
	\frac{\lambda_j}{n_T-1} K
	\right \}\right )
	\right )^- I^{(J), \top} y
	\end{align}
	where $A^-$ is defined as a generalized inverse of $A$.
	
	To apply Theorem~\ref{thrm:train_val}, we need to be able to characterize how the natural spline $\hat{g}(\boldsymbol{\lambda})$ that interpolates $\hat{g}(\boldsymbol{\lambda})|_{D^{(n_T)}}$ varies with $\boldsymbol{\lambda}$.
	To do this, recall that any piecewise cubic polynomial can be expressed using a B-spline basis of order 4.
	Then $\hat{g}(\boldsymbol{\lambda})$ can expressed as a linear combination of the normalized B-spline basis functions $B_{jm}(x | D^{(n_T)})$ for the knot sequence $x_{ij}$:
	\begin{align}
	\hat{g}(\boldsymbol{\lambda})
	= \sum_{j=1}^J \sum_{m=1}^{N_j} B_{jm}(x | D^{(n_T)}) \hat{\theta}_{jm}(\boldsymbol{\lambda}).
	\end{align}
	Using a normalized B-spline basis is convenient as DeBoor(CITE) showed that for any $x, x' \in [0,1]^J$ and any normalized B-spline basis, there exists a number $\kappa_{4, \infty}$ such that
	\begin{align}
	\frac{
		g_j(x' | \boldsymbol{\theta})}{
		g_j(x | \boldsymbol{\theta})}
	\le
	\kappa_{4, \infty}.
	\label{eq:cond_b_spline}
	\end{align}
	%JF check wording pelase
	$\kappa_{4, \infty}$ is also called the $L_\infty$-condition number for B-splines.
	Using the result in \eqref{eq:cond_b_spline}, we can bound the difference between fitted functions at $\boldsymbol{\lambda}, \boldsymbol{\lambda}' \in \Lambda$ by
	\begin{align}
	\left | \hat{g}(\boldsymbol{\lambda})(x) - \hat{g}(\boldsymbol{\lambda}')(x) \right |
	& = \left|
	\sum_{j=1}^J \sum_{m=1}^{N_j} B_{jm}(x | D^{(n_T)}) \left(
	\hat{\theta}_{jm}(\boldsymbol{\lambda}) - \hat{\theta}_{jm}(\boldsymbol{\lambda}')
	\right)
	\right| \\
	& \le \kappa_{4, \infty}
	\min_{i \in D^{(n_T)}} \left|
	\sum_{j=1}^J \sum_{m=1}^{N_j} B_{jm}(x_i | D^{(n_T)}) \left(
	\hat{\theta}_{jm}(\boldsymbol{\lambda}) - \hat{\theta}_{jm}(\boldsymbol{\lambda}')
	\right)
	\right| \\
	& \le \kappa_{4, \infty}
	\min_{i \in D^{(n_T)}} \left|
	\hat{g}(\boldsymbol{\lambda})(x_i)
	- \hat{g}(\boldsymbol{\lambda}')(x_i)
	\right|\\
	& \le \kappa_{4, \infty}
	\min_{i \in D^{(n_T)}} \left\|
	\sup_{\lambda\in \Lambda} \nabla_{\lambda} \hat{g}(\boldsymbol{\lambda})(x_i)
	\right \|_2
	\left\|
	\boldsymbol{\lambda} - \boldsymbol{\lambda}'
	\right\|_2
	\end{align}
	We can bound the gradient of the fitted values with respect to $\boldsymbol{\lambda}$ using the derivative of projection matrices \citep{zhang2017matrix}.
	First, note that
	\begin{align}
	\hat{g}(\boldsymbol{\lambda})|_{D^{(n_T)}}
	= \left(
	...
	\right)
	\end{align}
	where $P_\lambda$ is the projection matrix
	\begin{align}
	P_\lambda = A(\boldsymbol{\lambda})
	\left(
	I^{(J)\top}I^{(J)} + \diag \left (\frac{\lambda_j}{n_T-1} K \right )
	\right)^-
	A^\top(\boldsymbol{\lambda})
	\end{align}
	and \begin{align}
	A(\boldsymbol{\lambda}) = \left(
	\begin{matrix}
	I^{(J)} \\
	\diag \left (\sqrt{
	\frac{\lambda_j}{n_T - 1}
	...
	\right)
	\end{align}
	Then $\frac{\partial}{\partial \lambda_j} P_\lambda = Z_\lambda + Z_\lambda^\top$ where
	\begin{align}
	Z_\lambda =
	(I - P_\lambda)
	\left(
	\begin{matrix}
	\boldsymbol{0} \\
	\diag(\{\boldsymbol{0}, ..., \boldsymbol{0},
	\sqrt{\frac{\lambda_j}{n_T - 1}} K^{1/2},
	...
	Thus
	\begin{align}
	\frac{\partial}{\partial \lambda_j} \hat{g}(\boldsymbol{\lambda})|_{D^{(n_T)}}
	= \left(
	\begin{matrix}
	\boldsymbol{I} & \boldsymbol{0}
	\end{matrix}
	\right)
	\left( Z_\lambda + Z_\lambda^\top \right )
	\left(
	\begin{matrix}
	y \\
	\boldsymbol{0}
	...
	\right).
	\end{align}
	Using Cauchy-Schwarz (\textcolor{red}{is this right terminology?}), we have for any $x_i \in D^{(n_T)}$
	\begin{align}
	\sup_{\lambda\in \Lambda}
	\left |
	\frac{\partial}{\partial \lambda_j} \hat{g}(\boldsymbol{\lambda})(x_i)
	\right |
	\le \sqrt{\frac{n_T - 1}{\lambda_{\min}}}
	\left \|K^{1/2} \right \|_2
	\|A(\boldsymbol{\lambda})^-\|_2
	\|y\|_2.
	\end{align}
	The spectral norm of $A(\boldsymbol{\lambda})^-$ is bounded above by the minimum non-zero singular value of $A(\boldsymbol{\lambda})$.
	Since $A(\boldsymbol{\lambda})$ is a concatenation of matrices, its minimum non-zero singular value is bounded below by the minimum non-zero singular value of the submatrix $I^{(J)}$.
	It is easy to show that the singular values of $I^{(J)}$ are all equal to $\sqrt{J}$.
	Thus we finally reach our Lipschitz result: the fitted function $\hat{g}(\boldsymbol{\lambda})$ is Lipschitz with the constant
	%JF notation needs fixing!
	\begin{align}
	C_{\Lambda, D^{(n_T)}}(x) = C_{\Lambda, D^{(n_T)}} \sqrt{\frac{n_T - 1}{\lambda_{\min}}}
	\quad \forall x \in [0,1]^J
	\label{eq:sobolev_lipschitz}
	\end{align}
	where
	$$ C_{\Lambda, D^{(n_T)}} = \kappa_{4, \infty} \sqrt{\frac{n_T - 1}{\lambda_{\min}}}
	\left \|K^{1/2} \right \|_2
	\|A(\boldsymbol{\lambda})^-\|_2
	\|y\|_2.
	$$
	...
	&\le \left\Vert g^* - \hat{g}^{(n_T)}( \tilde{\boldsymbol{\lambda}} | T) \right \Vert^2_{V}\\
	& + O_p\left (
	\frac{J \kappa \log(C_{\Lambda, D^{(n_T)}} n + 1)}{n_{V}}
	\right ) \\
	& + O_p\left(
	\sqrt{\frac{J \kappa \log(C_{\Lambda, D^{(n_T)}}  n + 1)}{n_{V}}\left\Vert g^* - \hat{g}^{(n_T)}( \tilde{\boldsymbol{\lambda}} | T)\right\Vert_{V}^2}
	\right ).
	\end{split}
	\end{align}
\end{example}
