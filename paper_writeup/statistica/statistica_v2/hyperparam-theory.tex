...
%	As the minimax rate for estimating additive models $g^*(x) = \sum_{j=1}^J g_j(x_{ij})$ with finite 2nd-order Sobolev norm is $O_p(n^{-4/5})$ \citep{horowitz2006optimal}, we see that the oracle error \eqref{eq:asym_train_val_oracle_risk} will asymptotically dominate the additional error incurred from tuning over the penalty parameters.
	...
	\citet{van2015penalized} showed that \eqref{eq:smoothing_spline} attains the minimax rate of $O_p(n^{-4/5})$ \citep{horowitz2006optimal} if the penalty parameters shrink at the rate of $O_p(n^{-4/5})$.
	...
	Moreover, as long as $\lambda_{\min}$ shrinks at a polynomial rate such that some penalty parameter in $\Lambda$ attains the minimax rate, the model selected via training/validation split will also attain the minimax rate.
\end{example}
