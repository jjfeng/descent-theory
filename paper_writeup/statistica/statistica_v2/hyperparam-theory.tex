...
Let us walk through a simple example with ridge penalties.
\begin{example}[Multiple ridge penalties]
	...
	\coloneqq 
	\frac{1}{2}
	\sum_{i=1}^n \left(y_i -  \sum_{j=1}^J x_{i}^{(j)} \boldsymbol{\theta}^{(j)} \right )^2
	...
	\right)$$
	and $\ell(x) = \sum_{j=1}^J \|x^{(j)}\|_2$.
	By Theorem~\ref{thrm:train_val}, for some absolute constant $c > 0$, we have with high probability
	\begin{align}
	\begin{split}
	\left\Vert g^* - \hat{g}^{(n_T)}( \hat{\boldsymbol{\lambda}} | T) \right\Vert _{V}^2
	&\le \left\Vert g^* - \hat{g}^{(n_T)}( \tilde{\boldsymbol{\lambda}} | T) \right \Vert^2_{V}\\
...
\begin{example}[Sobolev penalties]
	The Sobolev penalty is a nonparametric functional.
