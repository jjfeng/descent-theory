...
It is a special case of Theorem~\ref{thrm:jean_cv} in the Appendix, which is a simple extension of Theorem 3.5 in \citet{lecue2012oracle}.
...
The first assumption bounds the tail behavior of the fitted models and is similar to that in \citet{lecue2012oracle}.
\begin{assump}
...
\begin{theorem}
	...
	\ge
	1 + \sum_{k=1}^{\infty}
	k\Pr_{D^{(n_T)}}\left(\|C_\Lambda(\cdot |D^{(n_{T})})\|_{L_{\psi_{2}}}\ge2^{k}\sigma_{0}\right).
	\end{align}
...
First, Assumption~\ref{assump:tail_margin} is quite strict in that it must hold for all datasets $D$ and hyper-parameters $\Lambda$.
...
%JF I dont like this topic sentence. paragraph seems out of place
...
If $\Lambda$ is of the form \eqref{thrm:lambda_range}, we find that the Lipschitz function $C_\Lambda(x | T)$ scales with some polynomial of $n$ for many penalized regression examples, such as those in Sections \ref{sec:param_add_models} and \ref{sec:nonparam_smooth}.
That is, there exist a function $C_\Lambda(x | T)$ and $\kappa \ge 0$ such that
\begin{equation}
\label{thrm:lipschitz_pen_reg}
\left \| \hat{g}^{(n_T)}(\boldsymbol{\lambda}|T)(x) - \hat{g}^{(n_T)}(\boldsymbol{\lambda}'|T)(x) \right \|_{L_{\psi_1}}
\le C_\Lambda(x| T) n^\kappa \left \| \boldsymbol{\lambda} - \boldsymbol{\lambda}' \right \|_2 \quad \forall \boldsymbol{\lambda}, \boldsymbol{\lambda}' \in \Lambda.
\end{equation}
In our examples, $\kappa$ typically grows linearly in $t_{\min}$.
Plugging \eqref{thrm:lipschitz_pen_reg} into Theorem~\ref{thrm:train_val}, we immediately see that the remainder term depends linearly on $\kappa$ as the the remainder term depends on the logarithm of the Lipschitz function.
The formulation in Theorem~\ref{thrm:kfold} more abstract.
In our Lasso example, we find that a similar relationship holds between the remainder term and $\kappa$.
...
\begin{lemma}
	...
	\end{equation}
	where $I$ is a $p \times p$ identity matrix and $A \succeq B$ means that $A - B$ is a positive semi-definite matrix.
