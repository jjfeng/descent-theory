...
This theorem requires minimal assumptions -- it only requires that $\epsilon$ are sub-gaussian random variables and the following Lipschit assumption:
\begin{assump}
...
Our oracle inequality depends on an additional assumption that bounds the tail behavior of the fitted models.
The assumption is based on that in \citet{lecue2012oracle}.
\begin{assump}
...
With these assumptions, we establish an oracle inequality that bounds the risk of the fitted model from cross-validation.
...
If $\Lambda$ is of the form \eqref{thrm:lambda_range}, we find that the Lipschitz function $C_\Lambda(x | T)$ scales with some $n^{O_p(t_{\min})}$ for many penalized regression examples, such as those in Sections \ref{sec:param_add_models} and \ref{sec:nonparam_smooth}.
So for the training-validation split procedure, we find that the remainder term depends linearly on $t_{\min}$.
For cross-validation, we find a similar relationship between $t_{\min}$ and the remainder term.
...
\begin{lemma}
	...
	Suppose Conditions~\ref{condn:nonsmooth1} and \ref{condn:nonsmooth2} are satisfied by some $\Lambda_{smooth}\subseteq\Lambda$.
