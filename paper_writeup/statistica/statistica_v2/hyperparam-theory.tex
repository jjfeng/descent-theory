...
\begin{assump}
	...
	%JF what is the ordering of conditions here?!
...
\begin{example}[Sobolev penalties]
	...
	To accomplish this, we use Theorem 2.1 in \citet{green1993nonparametric} which states that the second derivatives at each of the covariates $\{1/k, ..., 1\}$ is equal to $\hat{\gamma}_j = R^{-1} Q^\top \hat{g}_j$.
	Then the value of the natural spline at any point $t$ is given by
	\begin{align}
	g_j(t) = \frac{(t - t_L) g_R + ...}{h} + ...
	\end{align}
	Thus we have the Lipschitz relation
	\begin{align}
	...
	\right \|_\infty
	\left(
	1 + \frac{c}{k^2} \sup_{\|a\|_\infty = 1} \|R^{-1} Q^\top a\|_\infty
	\right)\\
	& \le
	\left \|
	\left . \hat{\boldsymbol{\theta}}_j(\boldsymbol{\lambda}^{(1)} | D^{(n_T)}) - \hat{\boldsymbol{\theta}}_j(\boldsymbol{\lambda}^{(2)} | D^{(n_T)}) \right |_{D^{(n_T)}}
	...
	\begin{align}
	\ell \equiv 1 + \frac{c}{k^2} \sup_{\|a\|_\infty = 1} \|R^{-1} Q^\top a\|_\infty.
	\label{eq:sobolev_ell}
	\end{align}
	Moreover, the Hessian of the objective function is
	\begin{align}
	...
	\right \} \right )
	\end{align}
	which has minimum eigenvalue $m(D^{(n_T)})$ for all $\boldsymbol{\lambda} \in \Lambda$.
	...
	\begin{align}
	C_{\Lambda}(x|D^{(n_T)}) = n^{\kappa + 1} \frac{J^2}{m(D^{(n_T)})} C_{D^{(n_T)}}
	\label{eq:sobolev_lipschitz}
	\end{align}
	where $C = \ell^2 \sqrt{\frac{1}{2} \|\epsilon\|_T^2  + C_\Lambda^*}$.
	...
	&\le \left\Vert g^* - \hat{g}^{(n_T)}( \tilde{\boldsymbol{\lambda}} | T) \right \Vert^2_{V}\\
	& + O_p\left (
	\frac{J (\kappa + 1) \log( J (m(D^{(n_T)}))^{-1} C_{D^{(n_T)}} n + 1)}{n_{V}}
	\right ) \\
	& + O_p\left(
	\sqrt{\frac{J (\kappa + 1) \log(J (m(D^{(n_T)}))^{-1} C_{D^{(n_T)}} n + 1)}{n_{V}}\left\Vert g^* - \hat{g}^{(n_T)}( \tilde{\boldsymbol{\lambda}} | T)\right\Vert_{V}^2}
	\right ).
	\end{split}
	\end{align}
\end{example}
