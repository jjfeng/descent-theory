...
Done
\begin{enumerate}
	\item Update lemma 2, write a formal proof for lemma 2
\end{enumerate}

TODO
\begin{enumerate}
	\item Make examples for each of the lemmas - give example convergence rates
	\item Read tibshirani’s proof for moving for emp norm to 2 norm?
	\item Read massart's model selection theorem and see if it is helpful here.
	\item Reread the proof for the CV theorem
	\item Decide how we should proceed with reviewer 2's request: (1) argue that it is not desired and (2) give a corollary
	\item Update all the notation and check all the notation.
	\item Make the english flow better. Feels like it needs lots of rewriting… though maybe that’s not a good idea. Dunno.
\end{enumerate}
...
\end{equation}
\todo{I don't need to divide by number of obs in T?}
...
\todo{is reviewer2 referring to this?}
\[
...
\end{equation}
are satisfied with high probability, where $\| \cdot \|$ is some norm; $a$ is some nonnegative constant; and $\delta(J,n)$ is a function of the number of hyper-parameters $J$ to tune and the number of validation samples $n$.
\todo{is reviewer2 referring to this?}
...
\todo{cite myself? cite more recent snoek?}
...
\begin{theorem}
	...
	\todo{is the lower bound on CLambda reasonable?}
...
\begin{theorem}
	...
	Let $G(x)$ be an envelope function such that $\sup_{g \in \mathcal{G}} |g (x)| \le G(x)$ and $\|G\|_{L_{\psi_2}} <\infty$.
...
Suppose for any dataset $D^{(n_T)}$, $\hat g (\boldsymbol{\lambda} | D^{(n_T)})$ satisfies for any $x$
\begin{align}
\left | \hat g (\boldsymbol{\lambda}^{(1)} | D^{(n_T)})(x) - \hat g (\boldsymbol{\lambda}^{(2)} | D^{(n_T)})(x) \right |
\le G(x) \|\boldsymbol{\lambda}^{(1)} - \boldsymbol{\lambda}^{(2)}\|_2.
\end{align}
	...
	+ \textcolor{red}{c \frac{(1+a)^2}{a} \frac{J G \log (GC_\Lambda \Delta_{\Lambda} ) \log n_V }{n_V}.}
	\end{align*}
	%JF inf not min
\end{theorem}

$\|G\|_{L_{\psi_2}} < \infty$ is a joint condition on $X$ and $\mathcal{G}$....

$\|\epsilon\|_{L_{\psi_2}} < \infty$ states that $\epsilon$ is sub-gaussian.
