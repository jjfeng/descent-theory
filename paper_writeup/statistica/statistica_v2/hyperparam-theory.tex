...
We now generalize the results to nonparametric additive models.
...
We use two simulations to isolate these two behaviors.
...
(For instance, if we wanted a single penalty parameter, we would constrain $\lambda_j$ for $j=1,...,J$ to be the same value.) 
The penalty parameters are tuned using a training/validation split.

\textbf{Simulation 1}: The true function is the sum of identical univariate sinusoids
\begin{align}
g_j^*(x_j) &= \sin(x_j) \quad \text{for } j = 1,...,J.
\label{eq:sim1}
\end{align} 
Since the univariate functions are all the same in \eqref{eq:sim1}, the oracle risk should be roughly as we increase the number of free penalty parameters.
...
\end{align}
should grows linearly in $J$ for this simulation setup.

\textbf{Simulation 2}: The true function is the sum of sinusoid functions with increasing frequency
\begin{align}
g_j^*(x_j) = \sin(x_j * 1.2^{j - 4}) \quad \text{for } j = 1,...,J.
\label{eq:sim2}
\end{align}
Since the Sobolev norms of $g_j$ increase with $j$, we expect that the penalty parameters $\tilde{\boldsymbol{\lambda}}$ that attain the oracle risk to be monotonically decreasing, e.g. $\tilde{\lambda}_1 > ... > \tilde{\lambda}_J$. 
...
For both simulations, we used $J = 8$ features and fit the models using 200 training and 200 validation samples. We considered 1, 2, 4, and 8 free penalty parameters, where the penalty parameter structure was nested. That is, when considering $k$ free penalty parameters, we constrained $\{\lambda_{8\ell/k + j} \}_{j = 1,...,8/k}$ to be equal, for $\ell= 0,...,k - 1$. Each simulation setting was replicated forty times.
...
Interestingly, performing linear regression with the single parameter case gives a slope of 1.45 with standard error 0.14.
This suggests that our oracle inequality might not be tight for the case where there is only a single penalty parameter.

For Simulation 2, we get the opposite behavior -- the validation loss of the selected model decreases as the number of penalty parameters increases (Figure~\ref{fig:simulations}(b)).
By plotting the error of the oracle model, we can see that the validation loss of the selected model decreases because the error of the oracle model is decreasing at a faster rate than the rate at which the additional error term \eqref{eq:asym_train_val_theorem1} grows.

These simulation results suggest that adding more hyper-parameters can improve model-estimation procedures.
Having a separate penalty parameter for each additive component allows GAMs to fit components with differing smoothness.
However if we know a priori that the additive components have the same smoothness, then it is best to use a single penalty parameter.
...
If the estimated models are Lipschitz in the hyper-parameters, the generalization error of the selected model is upper bounded by a combination of the oracle risk, a near-parametric term in the number of hyper-parameters, and the geometric mean of the two.
The results show that adding hyper-parameters can decrease the generalization error of the selected model if the oracle risk decreases by a sufficient amount.
In the semi- or non-parametric setting, the error incurred from tuning hyper-parameters is dominated by the oracle risk asymptotically; adding hyper-parameters will have a negligible effect on the generalization error of the selected model.
...
In our penalized regression examples, we showed that the estimated models satisfy the Lipschitz condition so our theoretical results apply.
We have shown that it can be desirable to fit models by with multiple penalties and penalty parameters, rather than the usual case with one or two penalty parameters.
