...
\begin{example}[Sobolev penalties]
	...
	For this example, suppose there is a $\delta > 0$ such that the covariates in the training set are chosen such that
	$$
	\min_{j=1,...,J} \min_{k,\ell \in D^{(n_T)}, x_{kj} \ne x_{\ell j} } |x_{kj} - x_{\ell j}| \ge \delta.
	$$
	We can re-express \eqref{eq:smoothing_spline} using vectors $g_j$ where $g_j(x_{ij})$ indicate the $j$-th function's value at $x_{ij}$.
	Suppose there are $n_j$ unique values for covariate $j$ in the training set.
	Then \eqref{eq:smoothing_spline} is equivalent to solving
	\begin{align}
	\{\hat{g}_j(\boldsymbol{\lambda}) \}
	& \in \argmin_{g_1 \in \mathbb{R}^{n_1} ,...,g_J \in \mathbb{R}^{n_J}}
	\frac{1}{2} \sum_{i\in D^{(n_T)}}
	\left(
	y_i - \sum_{j=1}^J g_{j}(x_i)
	\right)^2
	+ \sum_{j=1}^{J} \lambda_j g_j^\top K_j g_j
	\label{eq:sobolev_rewrite}
	\end{align}
	where the matrices $K_j$ are positive semi-definite matrices dependent on the set of unique $j$-th covariate values.
	The exact construction for $K_j$ is given in \citet{green1993nonparametric}.
	We can also rewrite this problem using matrix notation.
	Let $M$ be the binary matrix where each row of $M$, denoted $M_i$, is constructed such that $M_i g = \sum_{j=1}^J g_{j}(x_i)$.
	Then using matrix notation, we have
	\begin{align}
	\hat{g}(\boldsymbol{\lambda})
	& \in \argmin_{g \in \mathbb{R}^{n_1 + ... + n_J}}
	\frac{1}{2}
	\left \|
	y - M g
	\right \|^2
	...
	\end{align}
	Now suppose that there are a sufficient number of training observations and the covariates are sampled in such a way such that $M$ has rank $n$.
	Then \eqref{eq:matrix_sobolev} has a unique minimizer:
	\begin{align}
	\hat{g}(\boldsymbol{\lambda}) = \left(
	M^\top M + \diag \left (
	\left \{
	\lambda_j K_j
	\right \} \right )
	...
	To apply Theorem~\ref{thrm:train_val}, we need to be able to characterize how the natural splines $\hat{g}_j(\boldsymbol{\lambda})(\cdot)$ that interpolates $\hat{g}_j(\boldsymbol{\lambda})$ varies with $\boldsymbol{\lambda}$.
	To do this, recall that any piecewise cubic polynomial can be expressed using a B-spline basis of order 4.
	Then for each $j$, $\hat{g}_j(\boldsymbol{\lambda})$ can expressed as a linear combination of the normalized B-spline basis functions $B_{jm}(x)$ for a knot sequence based on the sampled $j$-th covariates:
	\begin{align}
	\hat{g}_j(\boldsymbol{\lambda})(x)
	= \sum_{m=1}^{N_j} B_{jm}(x | D^{(n_T)}) \hat{\theta}_{jm}(\boldsymbol{\lambda}).
	\end{align}
	Using a normalized B-spline basis is convenient as \citet{de1975local} proved that B-splines of order $k$ have condition number $\kappa_{k, \infty}$: for any knot sequence $t$ and any $N \ge 1$ normalized B-splines functions, define
	\begin{align}
	\sup_{N \ge 1} \sup_t 
	\frac{
	\sup_{\|a\|_\infty = 1} \| \sum_{n=1}^N a_n B_n(x) \|_\infty
	}{
	\inf_{\|a\|_\infty = 1} \| \sum_{n=1}^N a_n B_n(x) \|_\infty
	}
	= \kappa_{k, \infty}
	\le 2k9^{k-1}.
	\label{eq:cond_b_spline}
	\end{align}
%	Thus for any $x, x' \in [0,1]^J$ and any normalized B-spline basis, we have
%	\begin{align}
%	\frac{
...
%	$\kappa_{4, \infty}$ is also called the $L_\infty$-condition number for B-splines.
	...
	\right| \\
	& \le \sum_{j=1}^J \left|
	\sum_{m=1}^{N_j} B_{jm}(x | D^{(n_T)}) \left(
	\hat{\theta}_{jm}(\boldsymbol{\lambda}) - \hat{\theta}_{jm}(\boldsymbol{\lambda}')
	\right)
	\right| \\
	& \le \kappa_{4, \infty}
	...
	\sum_{m=1}^{N_j} B_{jm}(x_{ij} | D^{(n_T)}) \left(
	\hat{\theta}_{jm}(\boldsymbol{\lambda}) - \hat{\theta}_{jm}(\boldsymbol{\lambda}')
	\right)
	\right| \\
	& \le \kappa_{4, \infty}
	\sum_{j=1}^J
	\min_{i \in D^{(n_T)}} \left|
	\hat{g}_j(\boldsymbol{\lambda})(x_{ij})
	- \hat{g}_j(\boldsymbol{\lambda}')(x_{ij})
	\right|\\
	& \le \kappa_{4, \infty}
	\sum_{j=1}^J
	...
	\nabla_{\lambda} \hat{g}_j(\boldsymbol{\lambda})(x_{ij})
	\right \|_2
	\left\|
	\boldsymbol{\lambda} - \boldsymbol{\lambda}'
	\right\|_2
	\end{align}
	Using the closed form solution for $\hat{g}(\boldsymbol{\lambda})$, we can bound the gradient of the fitted values with respect to $\boldsymbol{\lambda}$ using Cauchy-Schwarz:
	\begin{align}
	\frac{\partial}{\partial\lambda_j} \hat{g}(\boldsymbol{\lambda})(x_{ij})
	\le
	\left(
	M^\top M + \diag \left (
	\left \{
	...
	\label{eq:bound_grad}
	\end{align}
	where $c_{x}$ is a constant only dependent on the covariates chosen in the training set.
	...
	&\le \left\Vert g^* - \hat{g}^{(n_T)}( \tilde{\boldsymbol{\lambda}} | T) \right \Vert^2_{V}\\
	& + O_p\left (
	\frac{J \kappa \log(\kappa_{4, \infty} J c_{x} \|y\|_2 n + 1)}{n_{V}}
	\right ) \\
	& + O_p\left(
	\sqrt{\frac{J \kappa \log(\kappa_{4, \infty} J c_{x} \|y\|_2 n + 1)}{n_{V}}\left\Vert g^* - \hat{g}^{(n_T)}( \tilde{\boldsymbol{\lambda}} | T)\right\Vert_{V}^2}
	\right ).
	\end{split}
	\end{align}
\end{example}
