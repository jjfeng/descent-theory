...
\begin{lemma}
	...
	- (y - g^*)^2
	 : P (\hat{g}(\boldsymbol{\lambda} | D^{(n_T)}) - g^*)^2 \le \epsilon , \boldsymbol{\lambda} \in \Lambda \},
	$$
	we have
	\begin{align}
	...
	\end{align}
	Moreover, if there is some $k > 0$ such that
	\begin{align}
	\|g^{*}-\hat{g}(\boldsymbol{\lambda}|D^{(n_{T})})\|_{L_{\psi_{2}}}\le c\left(\|g^{*}-\hat{g}(\boldsymbol{\lambda}|D^{(n_{T})})\|_{L_{2}}\right)^{k},
	\end{align}
	then we also have
	\begin{align}
