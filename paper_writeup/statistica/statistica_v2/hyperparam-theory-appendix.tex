...
%	\]
	...
	\right].
	$$
	Therefore we bound the norm of $\left. \nabla_{\theta}P_{j}(\boldsymbol{\theta}^{(j)})
	\right|_{\boldsymbol{\theta}=\hat{\boldsymbol{\theta}}(\lambda|T)}$ by rearranging \eqref{eq:grad_opt} and using the Cauchy-Schwarz inequality:
	\begin{align*}
	...
	\end{align}
	Plugging in the results from above and the assumption that the Hessian of the objective function is has a minimum eigenvalue of $m(T)$, then the norm of $\nabla_\lambda \hat{\boldsymbol{\theta}}(\boldsymbol{\lambda})$ is bounded by
	\begin{align}
	\left \|
	\left .
	\nabla_{\lambda}\hat{\boldsymbol{\theta}}^{(j)}(\boldsymbol{\lambda} | T)
	\right|_{\boldsymbol{\theta}=\hat{\boldsymbol{\theta}}(\lambda|T)}
	\right \|_2
	...
	\end{align}
	Finally the result \eqref{eq:param_add_lipschitz} follows as  $g_j(\boldsymbol{\theta})$ are Lipschitz in $\boldsymbol{\theta}$:
	\begin{align}
	\left |
