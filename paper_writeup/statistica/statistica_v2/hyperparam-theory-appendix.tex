...
\label{app:cv}
We now present the proof for Theorem \ref{thrm:kfold}.
It is an application of Theorem 3.5 in \citet{lecue2012oracle}, which we have reproduced below for convenience.
...
\begin{theorem}
	...
	Suppose there is a $d_{\min} \ge 0$ and a strictly increasing function $J$ such that $J^{-1}$ is strictly convex,
	the convex conjugate $\psi$ of $J^{-1}$ increases, $\psi(\infty ) = \infty$ and there exists $r \ge 1$ such that $\psi(x)/x^r$ decreases and
	\begin{equation}
	\label{eq:mitchell_dudley_bound}
	J(t) \ge \int_0^{2\sqrt{t}} H^{1/2}(u, \mathcal{Q}_t, \| \cdot \|_2) du +
	\frac{\log n_V}{\sqrt{n_V}} \int_0^{2\|G\|_{L_{\psi_1}}} H(u, \mathcal{Q}_t, \| \cdot \|_\phi) du
	\quad
	\forall t > t_{\min}
	\end{equation}
	...
	\right \|^2_{L_2}\\
	& + \frac{ac}{q}\left [
	\psi \left (\frac{2q^{r + 1}(1+a)}{a\sqrt{n_V}} \right ) \vee t_{\min}
	\right ]
	\end{split}.
	\end{align}
\end{theorem}

To prove Theorem \ref{thrm:kfold}, we will need the following lemma that describes the relationship between Lipschitz functions
\begin{lemma}
	Suppose the same conditions as Theorem~\ref{thrm:mitchell}.
	Suppose norm $\|\cdot \|$ satisfies
	\begin{align}
	\| XY \| \le \|X\|_*\|Y\|_*
	\label{eq:cauchy-like}
	\end{align}
	for some norm $\|\cdot \|_*$ for any random variables $X,Y$ and $\sup_{g \in \mathcal G}\|g(x)\|_*, \|\epsilon\|_* < \infty$.
	Moreover, suppose the Lipschitz assumption that for any $m \in \mathbb{N}$ and any dataset $D^{(m)}$, we have
	\begin{align}
	\left |\hat{g}^{(n_T)}(\boldsymbol{\lambda}^{(1)}|D^{(n_T)})(x) - \hat{g}^{(n_T)}(\boldsymbol{\lambda}^{(1)}|D^{(n_T)})(x) \right | \le G(x) \|\boldsymbol{\lambda}^{(1)} - \boldsymbol{\lambda}^{(2)}\|_2
	\end{align}
	We have
	\begin{align}
	N(\mathcal{Q}, u, \|\cdot \|)
	\le N \left (\Lambda, \frac{u}{\left (2 \|\epsilon\|_* + 6 \sup_{g \in \mathcal G} \left \|g(x) \right \|_* \right) \|G(x)\|_*}, \|\cdot \|_{2} \right)
	\end{align}
...
\left(\hat{g}^{(n_T)}(\boldsymbol{\lambda}^{(2)}|D^{(n_T)})(x) - \hat{g}^{(n_T)}(\boldsymbol{\lambda}^{(1)}|D^{(n_T)})(x)\right ) - \left(\hat{g}^{(n_T)}(\boldsymbol{\lambda}^{(2)}|D^{(n_T)})(x) - \hat{g}^{(n_T)}(\boldsymbol{\lambda}^{(1)}|D^{(n_T)})(x)\right )^2 \right \|\\
%JF falling off page
...
- \left (y - \hat{g}^{(n_T)}(\boldsymbol{\lambda}^{(2)}|D^{(n_T)})(x) \right) ^2 - \left (y - g^*(x) \right) ^2
\right \| \\
& \le  \left (2 \|\epsilon\|_* + 6 \sup_{g \in \mathcal G} \left \|g(x) \right \|_* \right) \|G(x)\|_* \|\boldsymbol{\lambda}^{(1)} - \boldsymbol{\lambda}^{(2)}\|_2.
\end{split}
\end{align}
\end{proof}
...
\begin{proof}
We check that the assumption \eqref{eq:cv_assump1} holds.
\begin{align}
& \left \| \left (y - \hat{g}^{(m)}(\boldsymbol{\lambda}|D^{(m)})(x) \right) ^2 - \left (y - g^*(x) \right) ^2  \right \|_{L_{\psi_1}} \\
& = \left \| 2 \epsilon \left (g^*(x) - \hat{g}(\boldsymbol{\lambda} | D^{(m)})(x) \right ) + \left( g^*(x) - \hat{g}(\lambda | D^{(m)})(x) \right)^2 \right \|_{L_{\psi_1}} \\
\label{eq:sum2}
& \le 3 \left \| \epsilon^2 \vee \left (g^*(x) - \hat{g}(\boldsymbol(\lambda) | D^{(m)})(x) \right )^2 \right \|_{L_{\psi_1}}
\end{align}
The last line follows by considering the case where $|\epsilon| > |g^*(x) - \hat{g}(\boldsymbol(\lambda) | D^{(m)})(x)|$ and when the inequality is in the other direction.
By triangle inequality, we then have
\begin{align}
& \left \| \left (y - \hat{g}^{(m)}(\boldsymbol{\lambda}|D^{(m)})(x) \right) ^2 - \left (y - g^*(x) \right) ^2  \right \|_{L_{\psi_1}} \\
& \le 3 \| \epsilon^2 \|_{L_{\psi_1}} + \| \left (g^*(x) - \hat{g}(\boldsymbol(\lambda) | D^{(m)})(x) \right )^2 \|_{L_{\psi_1}} \\
& = 3 \| \epsilon \|_{L_{\psi_2}}^2 + \| g^*(x) - \hat{g}(\boldsymbol(\lambda) | D^{(m)})(x)  \|_{L_{\psi_2}}^2
\end{align}
Since we assumed that $\|\epsilon\|_{L_{\psi_2}}$ and $\|G\|_{L_{\psi_2}}$ are finite, then clearly \eqref{eq:cv_assump1} holds.

Next, we check that assumption \eqref{eq:cv_assump2} holds.
\begin{align}
& \left \| \left (y - \hat{g}^{(m)}(\boldsymbol{\lambda}|D^{(m)})(x) \right) ^2 - \left (y - g^*(x) \right) ^2  \right \|_{L_2} \\
& = \left \| 2 \epsilon \left (g^*(x) - \hat{g}(\boldsymbol{\lambda} | D^{(m)})(x) \right ) + \left( g^*(x) - \hat{g}(\boldsymbol{\lambda} | D^{(m)})(x) \right)^2 \right \|_{L_2} \\
& \le \left (2 \|\epsilon\|_{L_2} + \|g^*(x) - \hat{g}(\boldsymbol{\lambda} | D^{(m)})(x)\|_{L_2} \right) \left \| g^*(x) - \hat{g}(\boldsymbol{\lambda} | D^{(m)})(x) \right \|_{L_2}
\end{align}
Since $\|f\|_{L_2} \le c \|f\|_{L_{\psi_2}}$, then clearly \eqref{eq:cv_assump2} holds.
