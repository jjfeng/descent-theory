...
+1\right].
$$
Moreover, if we have that $\forall Q\in\mathcal{Q}(D^{(n_{T})}),\|Q\|_{L_{\psi_{1}}}\le K$, then there is an absolute constant $c_1 > 0$ such that
$$
\gamma_{1}\left(\mathcal{Q}_{w}^{L_{2}}(D^{(n_{T})}),\|\cdot\|_{L_{\psi_{1}}}\right)	\le
c_1 J K \left[\log\left(
\frac{\Delta_{\Lambda}
	\left(\|\epsilon\|_{L_{\psi_{2}}} + K_2 w^{r/2}\right)
	\|C_\Lambda(x|D^{(n_{T})})\|_{L_{\psi_{2}}}}{K} + 1 \right)
+1\right].
$$
\end{lemma}
...
Using the above lemma, we can now define our functions $h$ and $J_\delta$.
Let
\begin{align}
h(D^{(n_T)}) = \|C_\Lambda(x|D^{(n_T)})\|_{\psi_1}.
\end{align}
In addition, let us suppose that $\delta \ge 1$.
Then for absolute constants, define
\begin{align}
\begin{split}
\label{eq:j_delta}
\mathcal{J}_{\delta}(w) & =
\frac{\log n_{V}}{\sqrt{n_{V}}}c_{2}JK\left[
\log\left(\frac{\Delta_{\Lambda}\delta K_2}{K} \left ( \|\epsilon\|_{L_{\psi_{2}}}+K \right )\right)
+\frac{r}{2}\log\left(w+1\right)
\right]\\
& +c_{3}\sqrt{w J}\left[\sqrt{\log\left(\Delta_{\Lambda}\|\epsilon\|_{L_{2}}\delta n_{v}+1\right)}+1\right].
\end{split}
\end{align}
