...
Let us also restate Assumption~\ref{assump:tail_margin} for the more general case.
\begin{assump}
...
2\left(
\|\epsilon\|_{L_{\psi_{2}}}
+ \sup_{\lambda \in \Lambda} \left \| g^*(x) - \hat{g}(\boldsymbol{\lambda}^{(1)} | D^{(n_T)})(x) \right \|_{L_{\psi_2}}
\right)
\|C_\Lambda (x | D^{(n_T)})\|_{L_{\psi_{2}}}
\|\boldsymbol{\lambda}^{(1)}-\boldsymbol{\lambda}^{(2)}\|_{2}
$$
as the dual of the $L_{\psi_1}$ norm is $L_{\psi_2}$.
From Assumption~\ref{assump:tail_margin} and the fact that $\|\epsilon\|_{L_{\psi_2}} = b < \infty$, we can show that
\begin{align}
\left \| g^*(x) - \hat{g}(\boldsymbol{\lambda}^{(1)} | D^{(n_T)})(x) \right \|_{L_{\psi_2}}
\le 
\sqrt{
\frac{1}{2} \left(
K_0 - 8b^2
...
\end{align}
Plug this in to attain the desired result.
...
\left[\sqrt{
\log\left(
\frac{b \Delta_{\Lambda}\|C_\Lambda(x|D^{(n_{T})})\|_{L_{2}}}{\sqrt{w}} + 1
\right)
}
+1\right]\\
\gamma_{1}\left(\mathcal{Q}_{w}^{L_{2}}(D^{(n_{T})}),\|\cdot\|_{L_{\psi_{1}}}\right)
...
%	& \le	2c\sqrt{w J}\int_{0}^{1}\sqrt{\log\left(\frac{8\Delta_{\Lambda}\left(\|\epsilon\|_{L_{2}}+\sqrt{w}\right)\|C_\Lambda(x|D^{(n_T)})\|_{L_{2}}+4\sqrt{w}}{2\sqrt{w}}\right)}+\sqrt{\log\frac{1}{v}}dv\\
	...
	\begin{align}
	\gamma_{1}\left(\mathcal{Q}_{w}^{L_{2}}(D^{(n_{T})}),\|\cdot\|_{L_{\psi_{1}}}\right)
	&\le c\int_{0}^{2K_0}\log N\left(\mathcal{Q}_{w}^{L_{2}}(D^{(n_{T})}),u,\|\cdot\|_{L_{\psi_{1}}}\right)du\\
	& \le 2cJK_0\left[
	\log\left(
	\frac{4 \Delta_{\Lambda}c_{K_0, b} \|C_\Lambda(x|D^{(n_T)})\|_{L_{\psi_{2}}}+2K_0}
	{K_0}
	\right)+1\right]
	\end{align}
\end{proof}
...
\begin{proof}
	...
	\sqrt{\left\Vert \epsilon \right \Vert_T^2 + 2 C^*_{\Lambda}}
	\left |\boldsymbol{\lambda}^{(1)}_j-\boldsymbol{\lambda}^{(2)}_j \right |.
	\end{align}
...
\begin{lemma}
	Suppose that $g_j(x|\boldsymbol{\theta})$ satisfies the Lipschitz
	condition in Lemma~\ref{lemma:param_add}.
	...
	satisfies the same Lipschitz condition as in Lemma~\ref{lemma:param_add}; that is, it satisfies \eqref{eq:param_add_lipschitz}.
...
\begin{proof}
	...
	\right ).
	\]
	Apply the Lemma~\ref{lemma:param_add} with $\Lambda=\mathcal{L}\left(\boldsymbol{p^{(i)},p^{(i+1)}}\right)$,
	and modify the proofs to take directional derivatives along the columns
	of $U^{(i)} = (U^{(i,1)}, ..., U^{(i,J)})$ instead.
	Then there is a Lipschitz constant $c_j>0$ independent of $i$ such that for all $i=1,2...$, such that 
	\begin{align}
	\left\Vert
	\boldsymbol{\hat{\beta}}^{(j)}(\boldsymbol{p}^{(i)}|T)
	-\boldsymbol{\hat{\beta}}^{(j)}(\boldsymbol{p}^{(i)}|T)
	\right\Vert _{2}
	\le c_j \left |{p}^{(i)}-{p}^{(i+1)}_j\right |.
	\end{align}
	...
	{p}^{(i)}_j-{p}^{(i+1)}_j
	\right |\\
	& = c_j \left |{\lambda}^{(1)}_j - {\lambda}^{(2)}_j\right |.
	\end{align*}
	Finally, following the same logic as in the proof of Lemma~\ref{lemma:param_add}, we have by the Lipschitz assumption that
	\[
	\left|g\left(x\mid\hat{\boldsymbol{\theta}}(\boldsymbol{\lambda}^{(1)}|T)\right)-g\left(x\mid\hat{\boldsymbol{\theta}}(\boldsymbol{\lambda}^{(2)}|T)\right)\right|
	\le
	\sqrt{\sum_{j=1}^J c_j^2 \ell_j^2(x_j)}
	\left \|\boldsymbol{\lambda}^{(1)}-\boldsymbol{\lambda}^{(2)} \right \|_{2}.
	\]
\end{proof}
...
\begin{proof}[Proof for Lemma~\ref{lemma:nonsmooth}]
	...
	As $g(x | \boldsymbol{\theta})$ is also continuous in $\boldsymbol{\theta}$, then for any $\boldsymbol{\lambda}^{(1)},\boldsymbol{\lambda}^{(2)}\in\Lambda$,
	we have
	\begin{align}
		\left |
		g\left(
		x \middle | \hat{\boldsymbol{\theta}}(\boldsymbol{\lambda}^{(1)})
		\right)
		-
		g\left(
		x \middle | \hat{\boldsymbol{\theta}}(\boldsymbol{\lambda}^{(2)})
		\right)
		\right |
		& = \lim_{i\rightarrow\infty}
		\left |
		g\left(
		x \middle | \hat{\boldsymbol{\theta}}(\boldsymbol{\lambda}^{(1,i)})
		\right)
		-
		g\left(
		x \middle | \hat{\boldsymbol{\theta}}(\boldsymbol{\lambda}^{(2,i)})
		\right)
		\right |\\
	...
	\end{align}
	where $c$ is the Lipschitz constant in Lemma~\ref{lemma:lipschitz_lambda_ext_c}.
\end{proof}
