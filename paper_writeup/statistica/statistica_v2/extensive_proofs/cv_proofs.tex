...
Then there exist constant $c_{1}>0$ and a constant $c_{2}>0$ that
depends on $K_{2},K,r,\|\epsilon\|_{\psi_{2}}$ such that

\begin{eqnarray*}
 &  & \mathbb{P}_{D^{(n)}}\left(\|\bar{g}(\hat{\boldsymbol{\lambda}}|D^{(n)})(x)-g^{*}(x)\|_{L_{2}}^{2}\right)\\
 & \le & (1+a)\inf_{\lambda\in\Lambda}\left[\mathbb{P}_{D^{(n_{T})}}\left(\|\hat{g}(\boldsymbol{\lambda}|D^{(n_{T})})(x)-g^{*}(x)\|_{L_{2}}^{2}\right)\right]\\
 &  & +c_{1}\frac{\left(1+a\right)^{2}}{a}\frac{J\log n_{V}}{n_{V}}c_{2}\log\left(c_{2}\frac{\left(1+a\right)}{a}\Delta_{\Lambda}J+1\right)\left(1+\tilde{h}(n_{T})\right).
\end{eqnarray*}


Suppose $K\ge1$.\end{thm}
\begin{proof}
By Lemma 1, 
\begin{eqnarray*}
 &  & \mathbb{P}_{D^{(n)}}\left(\|\bar{g}(\hat{\boldsymbol{\lambda}}|D^{(n)})(x)-g^{*}(x)\|_{L_{2}}^{2}\right)\\
...
we have
\[
\log\left(\frac{uA_{2,\delta}}{2}+\sqrt{uA_{1}}\right)\le c\log\left(uJ\left[\log\left(\Delta_{\Lambda}\|\epsilon\|_{L_{2}}\delta n_{v}+1\right)+1+Kr\right]\right)
\]
\begin{eqnarray*}
\psi_{\delta}(u) & \le & u\left[A_{0,\delta}+A_{1}2\log\left(\frac{uA_{2,\delta}}{2}+\sqrt{uA_{1}}\right)\right]+\left(\frac{u}{2}A_{2,\delta}\right)^{2}\\
 & \le & c_{2}u\frac{\log n_{V}}{\sqrt{n_{V}}}JK\left(1+r\right)\log\left(\left(1+r\right)J\Delta_{\Lambda}K_{2}K\|\epsilon\|_{L_{\psi_{2}}}u\delta\log n_{v}+1\right)\\
 &  & +c_{3}u^{2}J\left[\log\left(\Delta_{\Lambda}\|\epsilon\|_{L_{2}}\delta n_{v}+1\right)+1\right]
\end{eqnarray*}


Thus $\psi_{\delta}(u)/u^{2}$ decreases. So we set $r=2$. (second
line follows from lots of rearranging and simplifying)
...
\begin{eqnarray*}
 ...
 & \le & c_{2}\frac{1+a}{a}\frac{J\log n_{V}}{n_{V}}K\left(1+r\right)\log\left(\left(1+r\right)J\Delta_{\Lambda}K_{2}K\|\epsilon\|_{L_{\psi_{2}}}\frac{1+a}{a}\delta\right)\\
 &  & +c_{3}\left(\frac{1+a}{a}\right)^{2}\frac{J}{n_{V}}\left[\log\left(\Delta_{\Lambda}\|\epsilon\|_{L_{2}}\delta n_{v}+1\right)+1\right]\\
 & \le & c_{4}\left(\frac{1+a}{a}\right)^{2}\frac{J\log n_{V}}{n_{V}}K\left(1+r\right)\log\left(\frac{\left(1+a\right)\left(1+r\right)}{a}\Delta_{\Lambda}K_{2}K\|\epsilon\|_{L_{\psi_{2}}}J\delta+1\right)
\end{eqnarray*}
...
\begin{eqnarray*}
 ...
 & = & \sum_{k=1}^{\infty}\Pr\left(c\|G(x|D^{(n_{T})})\|_{L_{\psi_{2}}}\ge2^{k}\sigma\right)\left[kc_{4}\left(\frac{1+a}{a}\right)^{2}\frac{J\log n_{V}}{n_{V}}K\left(1+r\right)\log\left(\frac{\left(1+a\right)\left(1+r\right)}{a}\Delta_{\Lambda}K_{2}K\|\epsilon\|_{L_{\psi_{2}}}J\sigma+1\right)\right]\\
 & \le & \left[c_{4}\left(\frac{1+a}{a}\right)^{2}\frac{J\log n_{V}}{n_{V}}K\left(1+r\right)\log\left(\frac{\left(1+a\right)\left(1+r\right)}{a}\Delta_{\Lambda}K_{2}K\|\epsilon\|_{L_{\psi_{2}}}J\sigma+1\right)\right]\sum_{k=1}^{\infty}k\Pr\left(c\|G(x|D^{(n_{T})})\|_{L_{\psi_{2}}}\ge2^{k}\sigma\right)\\
 & \le & \left[c_{4}\left(\frac{1+a}{a}\right)^{2}\frac{J\log n_{V}}{n_{V}}K\left(1+r\right)\log\left(\frac{\left(1+a\right)\left(1+r\right)}{a}\Delta_{\Lambda}K_{2}K\|\epsilon\|_{L_{\psi_{2}}}J\sigma+1\right)\right]\tilde{h}(n_{T})
\end{eqnarray*}


Now let's finally plug in all the terms with $\sigma=1$. Then
\begin{eqnarray*}
 &  & \mathbb{P}_{D^{(n)}}\left(\|\bar{g}(\hat{\boldsymbol{\lambda}}|D^{(n)})(x)-g^{*}(x)\|_{L_{2}}^{2}\right)\\
 & \le & (1+a)\inf_{\lambda\in\Lambda}\left[\mathbb{P}_{D^{(n_{T})}}\left(\|\hat{g}(\boldsymbol{\lambda}|D^{(n_{T})})(x)-g^{*}(x)\|_{L_{2}}^{2}\right)\right]\\
 &  & +c_{2}\frac{\left(1+a\right)^{2}}{a}\left[\frac{J\log n_{V}}{n_{V}}K\left(1+r\right)\log\left(\frac{\left(1+a\right)\left(1+r\right)}{a}\Delta_{\Lambda}K_{2}K\|\epsilon\|_{L_{\psi_{2}}}J+1\right)\right]\left(1+\tilde{h}(n_{T})\right).
\end{eqnarray*}
