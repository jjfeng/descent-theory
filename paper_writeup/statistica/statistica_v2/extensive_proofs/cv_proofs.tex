...
to be a bit easier to handle:
\begin{eqnarray*}
\gamma_{1}\left(\mathcal{Q}_{\epsilon}^{L_{2}}(D^{(n_{T})}),\|\cdot\|_{L_{\psi_{1}}}\right) & \le & c_{1}JK\left[\log\left(\left(\frac{\Delta_{\Lambda}\|G(x|D^{(n_{T})})\|_{L_{\psi_{2}}}}{K}+1\right)\left(\|\epsilon\|_{L_{\psi_{2}}}+1\right)\left(c+1\right)\left(\epsilon^{r/2}+1\right)\right)+1\right]\\
 & \le & c_{1}JK\left[\log\left(\left(\frac{\Delta_{\Lambda}\|G(x|D^{(n_{T})})\|_{L_{\psi_{2}}}}{K}+1\right)\left(\|\epsilon\|_{L_{\psi_{2}}}+1\right)\left(c+1\right)\right)+\frac{r}{2}\log\left(\epsilon+1\right)+1\right]
\end{eqnarray*}
...
\[
\tilde{h}(n_{T})\ge\sum_{k=1}^{\infty}k\Pr\left(\|G(x|D^{(n_{T})})\|_{L_{\psi_{2}}}\ge2^{k}\sigma\right)
\]
...
\begin{eqnarray*}
 &  & \mathbb{P}_{D^{(n)}}\left(\|\bar{g}(\hat{\boldsymbol{\lambda}}|D^{(n)})(x)-g^{*}(x)\|_{L_{2}}^{2}\right)\\
 & \le & (1+a)\inf_{\lambda\in\Lambda}\left[\mathbb{P}_{D^{(n_{T})}}\left(\|\hat{g}(\boldsymbol{\lambda}|D^{(n_{T})})(x)-g^{*}(x)\|_{L_{2}}^{2}\right)\right]\\
 &  & +c_{1}\frac{\left(1+a\right)^{2}}{a}\frac{J\log n_{V}}{n_{V}}K\left[\log\left(\Delta_{\Lambda}\|\epsilon\|_{L_{\psi_{2}}}c\right)+K\frac{J\log n_{V}}{n_{V}}\right]\left(1+\tilde{h}(n_{T})\right)
\end{eqnarray*}
\end{thm}
\begin{proof}
By Lemma 1, 
...
To see this, we use the fact that $\log(\epsilon+1)\le\sqrt{\epsilon}$
for all $\epsilon>0$. This implies that 
\begin{eqnarray*}
\psi_{\delta}(u) & = & \sup_{v>0}uv-\mathcal{J}_{\delta}^{-1}(v)\\
 & = & \sup_{w>0}u\mathcal{J}_{\delta}(w)-w\\
 & = & \sup_{w>0}u\left[A_{0,\delta}+A_{1}\log(w+1)+A_{2,\delta}\sqrt{w}\right]-w\\
 & \le & \sup_{w>0}u\left[A_{0,\delta}+\left(A_{1}+A_{2,\delta}\right)\sqrt{w}\right]-w\\
 & \le & A_{0,\delta}u+\frac{\left(A_{1}+A_{2,\delta}\right)^{2}}{4}u^{2}
\end{eqnarray*}


Thus $\psi_{\delta}(u)/u^{2}$ decreases and so we choose $r=2$.

Let $q\rightarrow1$. Then

\begin{eqnarray*}
\lim_{q\rightarrow1}\epsilon_{q,\delta}(1/q) & = & \psi_{\delta}\left(\frac{2(1+a)}{a}\frac{1}{\sqrt{n_{V}}}\right)\vee\frac{1}{n_{V}}\\
 & \le & \left[\left(\frac{2(1+a)}{a}\right)\frac{\log n_{V}}{n_{V}}c_{1}JK\left[\log\left(\left(\frac{\Delta_{\Lambda}\delta}{K}+1\right)\left(\|\epsilon\|_{L_{\psi_{2}}}+1\right)\left(c+1\right)\right)+1\right]+\frac{\left(\frac{\log n_{V}}{\sqrt{n_{V}}}c_{1}JK+c_{2}\sqrt{J}\left[\sqrt{\log\left(\Delta_{\Lambda}\left[\|\epsilon\|_{L_{2}}\delta n_{v}+1\right]+1\right)}+1\right]\right)^{2}}{4}\left(\frac{2(1+a)}{a}\frac{u}{\sqrt{n_{V}}}\right)^{2}\right]\\
 & \le & c_{0}\left(\frac{1+a}{a}\right)^{2}\frac{J\log n_{V}}{n_{V}}\left[K^{2}\frac{J\log n_{V}}{n_{V}}+K\log\left(\Delta_{\Lambda}\delta\|\epsilon\|_{L_{\psi_{2}}}c\right)\right]
\end{eqnarray*}


where the last line assumes that the elements in the $\log$ terms
are all larger than one, $K,c\ge1$.

Finally, let's do the summation: 
\begin{eqnarray*}
 &  & \sum_{k=1}^{\infty}\Pr\left(c\|G(x|D^{(n_{T})})\|_{L_{\psi_{2}}}\ge2^{k}\sigma\right)\epsilon_{1,2^{k}\sigma}(1)\\
 & = & \sum_{k=1}^{\infty}\Pr\left(c\|G(x|D^{(n_{T})})\|_{L_{\psi_{2}}}\ge2^{k}\sigma\right)\left[c_{1}\left(\frac{1+a}{a}\right)^{2}\frac{J\log n_{V}}{n_{V}}\left[K^{2}\frac{J\log n_{V}}{n_{V}}+kK\log\left(\Delta_{\Lambda}\|\epsilon\|_{L_{\psi_{2}}}c\right)\right]\right]\\
 & \le & c_{1}\left(\frac{1+a}{a}\right)^{2}\frac{J\log n_{V}}{n_{V}}\left[K^{2}\frac{J\log n_{V}}{n_{V}}+K\log\left(\Delta_{\Lambda}\|\epsilon\|_{L_{\psi_{2}}}c\right)\right]\sum_{k=1}^{\infty}k\Pr\left(c\|G(x|D^{(n_{T})})\|_{L_{\psi_{2}}}\ge2^{k}\sigma\right)\\
 & \le & c_{1}\left(\frac{1+a}{a}\right)^{2}\frac{J\log n_{V}}{n_{V}}\left[K^{2}\frac{J\log n_{V}}{n_{V}}+K\log\left(\Delta_{\Lambda}\|\epsilon\|_{L_{\psi_{2}}}c\right)\right]\tilde{h}(n_{T})
\end{eqnarray*}


Now let's finally plug in all the terms.
\begin{eqnarray*}
 &  & \mathbb{P}_{D^{(n)}}\left(\|\bar{g}(\hat{\boldsymbol{\lambda}}|D^{(n)})(x)-g^{*}(x)\|_{L_{2}}^{2}\right)\\
 & \le & (1+a)\inf_{\lambda\in\Lambda}\left[\mathbb{P}_{D^{(n_{T})}}\left(\|\hat{g}(\boldsymbol{\lambda}|D^{(n_{T})})(x)-g^{*}(x)\|_{L_{2}}^{2}\right)\right]\\
 &  & +c_{2}\frac{\left(1+a\right)^{2}}{a}\frac{J\log n_{V}}{n_{V}}K\left[\log\left(\Delta_{\Lambda}\|\epsilon\|_{L_{\psi_{2}}}c\right)+K\frac{J\log n_{V}}{n_{V}}\right]\left(1+\tilde{h}(n_{T})\right)
\end{eqnarray*}
