...
value theorem. We assume that the original training criterion is locally
strongly convex around its minimizer. 
\item We extend the result to parametric models where the training criterion
has nonsmooth penalties.
\end{enumerate}
...
Let $U_{\lambda}$ be an orthonormal basis of $\Omega$.
...
Suppose $_{U}\nabla_{\beta}^{2}L_{T}(U\boldsymbol{\beta}|\boldsymbol{\lambda})$
...
\[
\left._{U}\nabla_{\beta}^{2}L_{T}(U\boldsymbol{\beta}|\boldsymbol{\lambda})\right|_{\beta=\hat{\boldsymbol{\beta}}_{\lambda}(w)}\succeq mI
\]
...
and $U$ is an orthonormal matrix, the result follows.
\end{document}
